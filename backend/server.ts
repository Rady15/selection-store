...
// Gift cards: purchasing a gift card issues a real single-use fixed-discount
// coupon voucher stored in the database (redeemable at checkout like any coupon).
app.post('/api/gift-cards', rateLimit({ windowMs: 60 * 60 * 1000, max: 10 }), (req, res) => {
  const amount = Number(req.body?.amount);
  const recipient = String(req.body?.recipient || '').trim().toLowerCase();
  const allowedAmounts = [100, 200, 300, 500];
  if (!allowedAmounts.includes(amount)) {
    return res.status(400).json({ error_ar: 'قيمة بطاقة الهدية غير صالحة', error_en: 'Invalid gift card amount' });
  }
  if (!/^\S+@\S+\.\S+$/.test(recipient)) {
    return res.status(400).json({ error_ar: 'البريد الإلكتروني للمستلم غير صالح', error_en: 'Invalid recipient email address' });
  }
  ...
  res.status(201).json({
    success: true,
    gift_card: { code: coupon.code, amount, recipient, valid_until: validUntil }
  });
});
...
app.post('/api/newsletter/subscribe', rateLimit({ windowMs: 60 * 60 * 1000, max: 10 }), (req, res) => {
  ...
  if (!email || !/^\S+@\S+\.\S+$/.test(String(email).trim())) return res.status(400).json({ error_ar: 'البريد الإلكتروني غير صالح', error_en: 'Invalid email address' });
  const sub = db.addNewsletterSubscriber(String(email).trim());
...
app.get('/api/admin/payment-gateways', (req, res) => {
  res.json(db.getPaymentGateways(false));
});

app.get('/api/admin/payment-gateways/:id', (req, res) => {
  const gateway = db.getPaymentGateway(req.params.id, false);
  if (!gateway) return res.status(404).json({ error: 'Payment gateway not found' });
  res.json(gateway);
});

app.put('/api/admin/payment-gateways/:id', (req, res) => {
  try {
    const updates = req.body || {};
    // Never allow setting secret_key or webhook_secret to a masked placeholder.
    // If client sends secret_key/webhook_secret, assume it's the real value.
    // If client omits them, don't change them.
    const allowed = ['enabled', 'mode', 'currency', 'publishable_key', 'secret_key', 'webhook_secret', 'supported_methods', 'additional_settings'];
    const payload: any = { id: req.params.id };
    for (const key of allowed) {
      if (updates[key] !== undefined) payload[key] = updates[key];
    }
    const gateway = db.savePaymentGateway(req.params.id, payload);
    res.json(db.getPaymentGateway(req.params.id, false));
  } catch (err: any) {
...
app.post('/api/admin/payment-gateways/:id/test', async (req, res) => {
  try {
    const gateway = db.getPaymentGateway(req.params.id, true);
    if (!gateway) return res.status(404).json({ error: 'Payment gateway not found' });
    
    let testResult: { status: 'connected' | 'error' | 'untested'; message: string } = { status: 'error', message: 'Unknown gateway' };
    
    if (gateway.id === 'stripe') {
      const secretKey = gateway.secret_key || process.env.STRIPE_SECRET_KEY;
      if (!secretKey) {
        testResult = { status: 'error', message: 'Stripe secret key not configured' };
      } else {
        try {
          ...
          testResult = { 
            status: 'connected', 
            message: `Connected to Stripe account: ${account.id} (${gateway.mode} mode)` 
          };
        } catch (err: any) {
          testResult = { status: 'error', message: `Stripe connection failed: ${err.message}` };
        }
      }
    ...
    // Update test status in the stored config
    db.savePaymentGateway(gateway.id, { test_status: testResult.status, test_message: testResult.message });
    res.json(testResult);
  } catch (err: any) {
