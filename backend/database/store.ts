...
const initialPaymentGateways: PaymentGatewayConfig[] = [
  {
    id: 'stripe',
    name_ar: 'بوابة الدفع الإلكتروني (Stripe - مدى / فيزا / أبل باي)',
    name_en: 'Stripe Online Payments (Mada / Visa / Apple Pay)',
    description_ar: 'معالجة الدفع بالبطاقات الائتمانية ومدى وApple Pay بأعلى معايير الأمان العالمية PCI DSS',
    description_en: 'Accept Mada, Apple Pay, Visa, and Mastercard with top-grade PCI DSS compliance',
    enabled: true,
    mode: (process.env.STRIPE_SECRET_KEY && process.env.STRIPE_SECRET_KEY.startsWith('sk_live_')) ? 'live' : 'test',
    currency: 'SAR',
    publishable_key: process.env.VITE_STRIPE_PUBLISHABLE_KEY || '',
    secret_key: process.env.STRIPE_SECRET_KEY || '',
    webhook_secret: process.env.STRIPE_WEBHOOK_SECRET || '',
    supported_methods: ['mada', 'visa', 'apple_pay']
  },
  {
    id: 'cod',
    name_ar: 'الدفع عند الاستلام (COD)',
    name_en: 'Cash On Delivery (COD)',
    description_ar: 'تمكين العملاء من دفع قيمة الطلب نقداً أو عبر الشبكة عند استلام الشحنة',
    description_en: 'Allow customers to pay in cash or via card upon parcel delivery',
    enabled: true,
    mode: 'live',
    currency: 'SAR',
  ...
  {
    id: 'tabby',
    name_ar: 'تابي - قسّمها على 4 دفعات (Tabby)',
    name_en: 'Tabby - Split in 4 Payments',
    description_ar: 'دفع مرن بدون فوائد أو رسوم إضافية متوافق مع الشريعة الإسلامية',
    description_en: 'Interest-free Buy Now Pay Later installments',
    enabled: false,
    mode: 'test',
    currency: 'SAR',
    supported_methods: ['tabby']
  },
  {
    id: 'tamara',
    name_ar: 'تمارا - قسّم فاتورتك (Tamara)',
    name_en: 'Tamara - Split in Installments',
    description_ar: 'خيار الدفع بالتقسيط المرن بدون فوائد للعملاء في السعودية والخليج',
    description_en: 'Flexible Sharia-compliant BNPL payment solution',
    enabled: false,
    mode: 'test',
    currency: 'SAR',
    supported_methods: ['tamara']
  },
  {
    id: 'paymob',
    name_ar: 'باي موب (Paymob Payment Gateway)',
    name_en: 'Paymob Gateway',
    description_ar: 'بوابة مدفوعات إقليمية تدعم مدى والبطاقات والمحافظ الإلكترونية',
    description_en: 'Regional payment gateway supporting cards, mada, and digital wallets',
    enabled: false,
    mode: 'test',
    currency: 'SAR',
    supported_methods: ['mada', 'visa']
  }
];
...
class Database {
  ...
  private migrateAdminUser() {
    ...
    if (!adminUser) {
      if (!adminPassword) {
        console.warn('[Security] ADMIN_PASSWORD is not configured; admin login is disabled.');
        return;
      }
    ...
    let changed = false;
    if (adminUser.email !== adminEmail) { adminUser.email = adminEmail; changed = true; }
    if (adminUser.password) {
      // One-time migration of legacy plaintext admin credentials.
      adminUser.password_hash = hashPassword(adminUser.password);
      delete adminUser.password;
      changed = true;
    }
    if (adminPassword && (!adminUser.password_hash || process.env.RESET_ADMIN_PASSWORD === 'true')) {
      adminUser.password_hash = hashPassword(adminPassword);
      changed = true;
    }
    if (changed) this.saveState();
  }

  addAuditLog(entry: { actor_user_id?: string; action: string; entity_type: string; entity_id?: string; metadata?: any }) {
    ...
    if (this.state.auditLogs.length > 5000) this.state.auditLogs.splice(0, this.state.auditLogs.length - 5000);
    this.saveState();
  }
  ...
  createOrder(orderData: Omit<Order, 'id' | 'order_number' | 'created_at' | 'status_history'>) {
    ...
    // Enforce coupon usage limits: redeem the coupon used on this order so
    // single-use vouchers (e.g. gift cards) cannot be reused.
    if (newOrder.coupon_code) {
      const used = this.state.coupons.find(c => c.code.toUpperCase() === String(newOrder.coupon_code).toUpperCase());
      if (used) used.usage_count = (used.usage_count || 0) + 1;
    }
    this.saveState();
    return newOrder;
  }
  ...
  getPaymentGateways(includeSecrets = false): PaymentGatewayConfig[] {
    return this.state.paymentGateways.map(g => {
      if (includeSecrets) return { ...g };
      return {
        ...g,
        secret_key: undefined,
        secret_key_configured: !!g.secret_key,
        secret_key_masked: maskSecret(g.secret_key),
        webhook_secret: undefined,
        webhook_secret_configured: !!g.webhook_secret,
        webhook_secret_masked: maskSecret(g.webhook_secret)
      };
    });
  }

  getPaymentGateway(id: string, includeSecrets = false): PaymentGatewayConfig | undefined {
    const gateway = this.state.paymentGateways.find(g => g.id === id);
    if (!gateway) return undefined;
    if (includeSecrets) return { ...gateway };
    return {
      ...gateway,
      secret_key: undefined,
      secret_key_configured: !!gateway.secret_key,
      secret_key_masked: maskSecret(gateway.secret_key),
      webhook_secret: undefined,
      webhook_secret_configured: !!gateway.webhook_secret,
      webhook_secret_masked: maskSecret(gateway.webhook_secret)
    };
  }

  savePaymentGateway(id: string, updates: Partial<PaymentGatewayConfig>): PaymentGatewayConfig {
    const idx = this.state.paymentGateways.findIndex(g => g.id === id);
    if (idx < 0) throw new Error('Payment gateway not found');
    const current = this.state.paymentGateways[idx];
    const next: PaymentGatewayConfig = { ...current, ...updates, id: current.id };
