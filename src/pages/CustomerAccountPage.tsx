...
export const CustomerAccountPage: React.FC<CustomerAccountPageProps> = ({ onNavigate }) => {
  ...
  if (user && isAdmin) {
    return (
      <div className="min-h-[70vh] flex items-center justify-center text-sm text-[#8EA8AA]">
  ...
  return (
    <div className="bg-[#061719] text-[#F3FBFB] min-h-screen py-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">

        {/* Profile Welcome Header */}
        <div className="p-8 rounded-3xl bg-gradient-to-r from-[#0A1E20] via-[#0B2528] to-[#0A1E20] border border-[#0B2528] shadow-2xl flex flex-col sm:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-[#8EA8AA] mt-0.5">{user.email} • {user.phone}</p>
            </div>
          </div>

          <div className="flex items-center gap-3">
            <div className="p-3 rounded-2xl bg-[#0E5257]/20 border border-[#0E5257]/40 text-center">
              <span className="text-[10px] text-[#8EA8AA] block">{t('رصيد نقاط سليكشن', 'Loyalty Points')}</span>
            ...
            <button
              ...
              className="p-3 rounded-2xl bg-[#061719] border border-[#0B2528] text-red-400 hover:bg-red-500/10 transition cursor-pointer"
        ...
        {/* Tab Navigation */}
        <div className="flex border-b border-[#0B2528] gap-6 text-sm font-bold overflow-x-auto">
          <button
            onClick={() => setActiveTab('orders')}
            className={`pb-3 transition cursor-pointer flex items-center gap-2 border-b-2 whitespace-nowrap ${activeTab === 'orders' ? 'border-[#6CC6C9] text-[#6CC6C9]' : 'border-transparent text-[#8EA8AA] hover:text-white'
              }`}
          >
          ...
          <button
            ...
            className={`pb-3 transition cursor-pointer flex items-center gap-2 border-b-2 whitespace-nowrap ${activeTab === 'wishlist' ? 'border-[#6CC6C9] text-[#6CC6C9]' : 'border-transparent text-[#8EA8AA] hover:text-white'
              }`}
          >
          ...
          <button
            ...
            className={`pb-3 transition cursor-pointer flex items-center gap-2 border-b-2 whitespace-nowrap ${activeTab === 'loyalty' ? 'border-[#6CC6C9] text-[#6CC6C9]' : 'border-transparent text-[#8EA8AA] hover:text-white'
              }`}
          >
          ...
          <button
            ...
            className={`pb-3 transition cursor-pointer flex items-center gap-2 border-b-2 whitespace-nowrap ${activeTab === 'profile' ? 'border-[#6CC6C9] text-[#6CC6C9]' : 'border-transparent text-[#8EA8AA] hover:text-white'
              }`}
          >
          ...
          <button
            ...
            className={`pb-3 transition cursor-pointer flex items-center gap-2 border-b-2 whitespace-nowrap ${activeTab === 'addresses' ? 'border-[#6CC6C9] text-[#6CC6C9]' : 'border-transparent text-[#8EA8AA] hover:text-white'
              }`}
          >
          ...
          <button
            ...
            className={`pb-3 transition cursor-pointer flex items-center gap-2 border-b-2 whitespace-nowrap ${activeTab === 'reviews' ? 'border-[#6CC6C9] text-[#6CC6C9]' : 'border-transparent text-[#8EA8AA] hover:text-white'
              }`}
          >
        ...
        {activeTab === 'orders' && (
          <div className="space-y-4">
            {orders.length === 0 ? (
              <p className="text-xs text-[#8EA8AA] text-center py-12">{t('لا توجد طلبات سابقة معالجة حتى الآن.', 'No past orders yet.')}</p>
            ) : (
              orders.map(ord => (
                <div key={ord.id} className="p-6 rounded-3xl bg-[#0A1E20] border border-[#0B2528] space-y-4">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-[#0B2528] pb-3 gap-2">
                    <div>
                      <span className="font-extrabold text-sm text-[#6CC6C9]">{ord.order_number}</span>
                      <span className="text-xs text-[#8EA8AA] block">
                  ...
                  <div className="space-y-2">
                    {ord.items.map((i: any, idx) => (
                      <div key={idx} className="flex justify-between text-xs text-[#B9D0D1]">
                  ...
                  <div className="flex justify-between items-center pt-3 border-t border-[#0B2528]">
                    <span className="text-xs text-[#8EA8AA]">{t('الإجمالي:', 'Total:')} <strong className="text-white text-sm font-extrabold">{formatPrice(ord.total_amount)}</strong></span>
                    <button
        ...
        {activeTab === 'loyalty' && (
          <div className="space-y-6">
            <div className="p-8 rounded-3xl bg-[#0A1E20] border border-[#0B2528] space-y-6 text-center max-w-xl mx-auto">
              ...
              <p className="text-xs text-[#B9D0D1] leading-relaxed">
              ...
              <div className="p-4 rounded-2xl bg-[#061719] border border-[#0B2528]">
                <span className="text-xs text-[#8EA8AA] block">{t('نقاطك القابلة للاستبدال', 'Redeemable Points')}</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-xl mx-auto">
              <div className="p-5 rounded-2xl bg-[#0A1E20] border border-[#0B2528] space-y-2">
                <div className="flex items-center gap-2">
                  <Star className="w-4 h-4 text-[#6CC6C9]" />
                  <span className="text-xs font-bold text-white">{t('كيف تكسب النقاط', 'How to Earn')}</span>
                </div>
                <p className="text-xs text-[#8EA8AA] leading-relaxed">
            ...
            <div className="p-6 rounded-3xl bg-[#0A1E20] border border-[#0B2528] max-w-xl mx-auto space-y-4">
              ...
              {loyaltyTransactions.length > 0 ? (
                <div className="space-y-3 max-h-80 overflow-y-auto">
                  {loyaltyTransactions.map((tx: any) => (
                    <div key={tx.id} className="flex items-center justify-between p-3 rounded-xl bg-[#061719] border border-[#0B2528]">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <span className="text-[10px] text-[#8EA8AA]">{new Date(tx.created_at).toLocaleDateString(language === 'ar' ? 'ar-SA' : 'en-US')}</span>
                        </div>
                      </div>
                ...
                <p className="text-xs text-[#8EA8AA] text-center py-6">
        ...
        {activeTab === 'profile' && (
          <div className="p-8 rounded-3xl bg-[#0A1E20] border border-[#0B2528] max-w-xl mx-auto space-y-6">
            ...
            <div className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-[#8EA8AA]">{t('الاسم الكامل', 'Full Name')}</label>
                <input
                  type="text"
                  value={profileName}
                  onChange={e => setProfileName(e.target.value)}
                  className="w-full p-3 rounded-xl bg-[#061719] border border-[#0B2528] text-[#F3FBFB] text-sm focus:border-[#6CC6C9] focus:outline-none transition"
                />
              </div>

              <div className="space-y-1.5">
                <label className="text-xs font-bold text-[#8EA8AA]">{t('البريد الإلكتروني', 'Email')}</label>
                <input
                  type="email"
                  value={profileEmail}
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-[#8EA8AA]">{t('رقم الهاتف', 'Phone Number')}</label>
                <input
                  type="tel"
                  value={profilePhone}
          ...
          <div className="space-y-6">
              ...
              <div className="p-6 rounded-3xl bg-[#0A1E20] border border-[#0B2528] space-y-4">
                ...
                <div className="space-y-4">
                  {eligibleItems.map(item => {
                    ...
                    return (
                      <div key={item.product_id} className="p-4 rounded-2xl bg-[#061719] border border-[#0B2528] space-y-3">
                        <div className="flex items-center gap-3">
                          {item.image && (
                            <img
                              src={item.image}
                              alt=""
                              className="w-14 h-14 rounded-xl object-cover border border-[#0B2528]"
                            />
                          )}
                          <div className="flex-1">
                            ...
                            <p className="text-[10px] text-[#8EA8AA]">{item.weight ? `${item.weight} • ` : ''}{item.quantity}x</p>
                          </div>
                        ...
                        <div className="flex items-center gap-1">
                          {[1, 2, 3, 4, 5].map(s => (
                            ...
                            >
                              <Star className={`w-6 h-6 ${s <= draft.rating ? 'text-[#6CC6C9] fill-[#6CC6C9]' : 'text-[#0B2528]'}`} />
                            </button>
                          ))}
                          <span className="text-[10px] text-[#8EA8AA] ms-2">{draft.rating}/5</span>
                        </div>
                        ...
                        <input
                          ...
                          className="w-full p-2.5 rounded-xl bg-[#0A1E20] border border-[#0B2528] text-xs text-white focus:border-[#6CC6C9] focus:outline-none transition"
                        />
            ...
            <div>
              ...
              {userReviews.length === 0 ? (
                <p className="text-xs text-[#8EA8AA] text-center py-12">{t('لا توجد تقييمات بعد', 'No reviews yet')}</p>
              ) : (
                userReviews.map(rev => (
                  <div key={rev.id} className="p-6 rounded-3xl bg-[#0A1E20] border border-[#0B2528] space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1">
                        {[1, 2, 3, 4, 5].map(s => (
                          <Star key={s} className={`w-4 h-4 ${s <= rev.rating ? 'text-[#6CC6C9] fill-[#6CC6C9]' : 'text-[#0B2528]'}`} />
                        ))}
                      </div>
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[#B9D0D1] mt-1">{rev.comment}</p>
                    </div>
                    <div className="flex items-center justify-between text-[10px] text-[#8EA8AA]">
                    ...
                    {rev.staff_reply_ar && (
                      <div className="p-3 rounded-xl bg-[#061719] border border-[#0B2528] text-xs text-[#B9D0D1]">
        ...
        {activeTab === 'addresses' && (
          <div className="space-y-4">
            ...
            {addresses.length === 0 ? (
              <div className="p-8 rounded-3xl bg-[#0A1E20] border border-[#0B2528] text-center">
                <MapPin className="w-12 h-12 text-[#0E5257] mx-auto mb-3" />
                <p className="text-xs text-[#8EA8AA]">{t('لا توجد عناوين محفوظة بعد', 'No saved addresses yet')}</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {addresses.map(addr => (
                  <div key={addr.id} className={`p-5 rounded-2xl bg-[#0A1E20] border space-y-3 ${addr.is_default ? 'border-[#6CC6C9]' : 'border-[#0B2528]'}`}>
                    ...
                    <div className="space-y-1 text-xs text-[#8EA8AA]">
                      <p className="text-[#B9D0D1]">{addr.full_name}</p>
                      <p>{addr.phone}</p>
                    ...
                    <div className="flex items-center gap-2 pt-2 border-t border-[#0B2528]">
                      {!addr.is_default && (
                        <button
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-lg bg-[#061719] border border-[#0B2528] text-[#8EA8AA] hover:text-white transition cursor-pointer"
                      >
                      ...
                      {deleteConfirmId === addr.id ? (
                        <div className="flex items-center gap-1 ml-auto">
                          ...
                          <button
                            ...
                            className="text-[10px] font-bold text-[#8EA8AA] hover:text-white cursor-pointer"
                          >
                            {t('إلغاء', 'Cancel')}
                          </button>
                        </div>
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-lg bg-[#061719] border border-[#0B2528] text-red-400 hover:bg-red-500/10 transition cursor-pointer ml-auto"
                        >
            ...
            {addressFormOpen && (
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-3xl bg-[#0A1E20] border border-[#0B2528] p-6 space-y-5">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="p-2 rounded-xl bg-[#061719] border border-[#0B2528] text-[#8EA8AA] hover:text-white transition cursor-pointer"
                    >
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                  ...
                  <div className="space-y-4">
                    {[
                      ...
                      <div key={field.key} className="space-y-1.5">
                        <label className="text-xs font-bold text-[#8EA8AA]">{field.label}</label>
                        <input
                          type={field.type}
                    ...
                    <label className="flex items-center gap-3 cursor-pointer">
                      ...
                      />
                      <span className="text-xs font-bold text-[#8EA8AA]">{t('تعيين كعنوان افتراضي', 'Set as default address')}</span>
                    </label>
                  </div>
