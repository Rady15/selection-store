...
export const ProductDetailPage: React.FC<ProductDetailPageProps> = ({ slug, onNavigate }) => {
  ...
  if (loading) {
    return (
      <div className="bg-[#061719] text-white min-h-screen py-20 flex justify-center items-center">
  ...
  if (!product) {
    return (
      <div className="bg-[#061719] text-white min-h-screen py-20 text-center space-y-4">
  ...
  return (
    <div className="bg-[#061719] text-[#F3FBFB] min-h-screen py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">

        {/* Breadcrumb Navigation */}
        <div className="flex items-center gap-2 text-xs text-[#8EA8AA]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div className="relative space-y-4">
            <div className="aspect-square rounded-3xl bg-[#0A1E20] border border-[#0B2528] overflow-hidden relative shadow-2xl">
              <img
              ...
              <div className="absolute top-4 right-4 flex gap-2">
                <button
                  ...
                  className={`p-3 rounded-full backdrop-blur-md transition cursor-pointer ${isLiked ? 'bg-red-500 text-white shadow-lg' : 'bg-[#061719]/70 text-[#B9D0D1] hover:text-white'
                    }`}
                ...
                <button
                  ...
                  className={`p-3 rounded-full backdrop-blur-md transition cursor-pointer ${shareOpen ? 'bg-[#6CC6C9] text-black' : 'bg-[#061719]/70 text-[#B9D0D1] hover:text-white'}`}
            ...
            {shareOpen && (
              <div className="absolute top-16 right-0 z-20 bg-[#0A1E20] border border-[#0B2528] rounded-2xl shadow-2xl p-3 w-56 space-y-1 animate-fade-in">
                <p className="text-[10px] font-bold text-[#8EA8AA] px-1 pb-1">
                  {t('مشاركة هذا المنتج', 'Share this product')}
                </p>
                <button
                  onClick={() => { setShareOpen(false); handleShare('clipboard'); }}
                  className="w-full flex items-center gap-2 px-2 py-2 rounded-xl text-xs text-[#F3FBFB] hover:bg-[#0B2528] transition cursor-pointer"
                >
                ...
                <button
                  ...
                  className="w-full flex items-center gap-2 px-2 py-2 rounded-xl text-xs text-[#F3FBFB] hover:bg-[#0B2528] transition cursor-pointer"
                >
                  <Share2 className="w-4 h-4 text-[#8EA8AA]" />
                  {t('X / تويتر', 'X / Twitter')}
                </button>
              </div>
            )}
              ...
              <div className="flex gap-3 overflow-x-auto pb-2">
                {product.images.map((img, idx) => (
                  <button
                    ...
                    className={`w-20 h-20 rounded-2xl overflow-hidden border-2 flex-shrink-0 transition cursor-pointer ${activeImageIdx === idx ? 'border-[#6CC6C9]' : 'border-[#0B2528] opacity-60'
                      }`}
                  >
            ...
            <div className="p-4 rounded-2xl bg-[#0A1E20] border border-[#0B2528] grid grid-cols-3 gap-3 text-center text-xs">
              <div>
                <span className="text-[#8EA8AA] block text-[10px]">{t('الارتفاع', 'Altitude')}</span>
                <span className="font-bold text-white">{product.altitude || '1800 - 2200m'}</span>
              </div>
              <div className="border-x border-[#0B2528]">
                <span className="text-[#8EA8AA] block text-[10px]">{t('السلالة', 'Variety')}</span>
                <span className="font-bold text-white">{product.variety || 'بوربون / أورثدكس'}</span>
              </div>
              <div>
                <span className="text-[#8EA8AA] block text-[10px]">{t('التقييم', 'SCA Score')}</span>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-[#B9D0D1] font-semibold">
            ...
            {/* Price & Rating */}
            <div className="flex items-center justify-between pb-4 border-b border-[#0B2528]">
              <div className="flex items-baseline gap-3">
                <span className="font-extrabold text-3xl text-[#6CC6C9]">
                  {formatPrice(unitPrice * quantity)}
                </span>
                {product.sale_price && (
                  <span className="text-sm text-[#8EA8AA] line-through">
              ...
              <div className="flex items-center gap-1.5 text-xs text-[#6CC6C9] font-bold">
                ...
                <span className="text-[#8EA8AA]">({product.review_count} {t('تقييم', 'reviews')})</span>
              </div>
            </div>

            {/* Tasting Notes */}
            <div className="space-y-2">
              <span className="text-xs font-bold text-[#8EA8AA] uppercase tracking-wider">{t('الإيحاءات العطرية والنكهات', 'Tasting Notes')}:</span>
              <div className="flex flex-wrap gap-2">
                {(language === 'ar' ? product.tasting_notes_ar : product.tasting_notes_en).map((note, idx) => (
                  <span key={idx} className="text-xs bg-[#0A1E20] text-[#6CC6C9] font-bold px-3 py-1.5 rounded-xl border border-[#0B2528]">
                    {note}
                  </span>
                ))}
              </div>
            ...
            {product.weight_options.length > 0 && (
              <div className="space-y-2">
                <span className="text-xs font-bold text-[#8EA8AA] uppercase tracking-wider">{t('اختر الوزن المطلوب', 'Select Bag Size')}:</span>
                  ...
                  {product.weight_options.map(w => (
                    <button
                      ...
                      className={`p-3 rounded-2xl text-xs font-bold transition border cursor-pointer ${selectedWeight === w.value
                        ...
                        : 'bg-[#0A1E20] text-[#B9D0D1] border-[#0B2528] hover:border-[#0E5257]'
                        }`}
                    >
            ...
            {product.grind_options.length > 0 && (
              <div className="space-y-2">
                <span className="text-xs font-bold text-[#8EA8AA] uppercase tracking-wider">{t('درجة الطحن المطلوبة', 'Grind Specification')}:</span>
            ...
            <div className="pt-2 space-y-3">
              {isOutOfStock ? (
                ...
                <div className="flex gap-3">
                  {/* Quantity selector */}
                  <div className="flex items-center border border-[#0B2528] rounded-2xl bg-[#0A1E20] px-2">
                    <button
            ...
            <div className="pt-4 border-t border-[#0B2528] grid grid-cols-2 gap-3 text-xs text-[#8EA8AA]">
              <div className="flex items-center gap-2">
        ...
        {product.flavor_profile && (
          <div className="p-8 rounded-3xl bg-[#0A1E20] border border-[#0B2528] grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 rounded-2xl bg-[#061719] border border-[#0B2528] space-y-2 text-xs text-[#B9D0D1]">
                <div className="flex justify-between border-b border-[#0B2528] pb-2">
        ...
        <div className="space-y-6">
          <div className="flex border-b border-[#0B2528] gap-6 text-sm font-bold">
            <button
              onClick={() => setActiveTab('details')}
              className={`pb-3 transition cursor-pointer border-b-2 ${activeTab === 'details' ? 'border-[#6CC6C9] text-[#6CC6C9]' : 'border-transparent text-[#8EA8AA] hover:text-white'
                }`}
            >
            ...
            <button
              ...
              className={`pb-3 transition cursor-pointer border-b-2 ${activeTab === 'reviews' ? 'border-[#6CC6C9] text-[#6CC6C9]' : 'border-transparent text-[#8EA8AA] hover:text-white'
                }`}
            >
            ...
            <button
              ...
              className={`pb-3 transition cursor-pointer border-b-2 ${activeTab === 'questions' ? 'border-[#6CC6C9] text-[#6CC6C9]' : 'border-transparent text-[#8EA8AA] hover:text-white'
                }`}
            >
          ...
          {activeTab === 'details' && (
            <div className="p-6 bg-[#0A1E20] border border-[#0B2528] rounded-3xl space-y-4 text-xs sm:text-sm text-[#B9D0D1] leading-relaxed">
