...
export const CoffeeFinderPage: React.FC<CoffeeFinderPageProps> = ({ onNavigate }) => {
  ...
  if (loading) {
    return (
      <div className="bg-[#061719] text-[#F3FBFB] min-h-screen flex items-center justify-center">
    ...
    <div className="bg-[#061719] text-[#F3FBFB] min-h-screen py-12">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 space-y-8">
        ...
        <div className="text-center space-y-3">
          ...
          {settings && (
            <p className="text-xs sm:text-sm text-[#B9D0D1] max-w-lg mx-auto">
        ...
        {questions.length > 0 && (
          <>
            {/* Progress Bar */}
            <div className="flex items-center justify-between max-w-md mx-auto text-xs font-bold text-[#8EA8AA]">
              {questions.map((q, i) => (
            ...
            <div className="w-full bg-[#0A1E20] h-2 rounded-full overflow-hidden border border-[#0B2528] max-w-md mx-auto">
              <div
        ...
        {/* Questions */}
        {currentQuestion && (
          <div className="p-8 rounded-3xl bg-[#0A1E20] border border-[#0B2528] space-y-6 animate-fade-in" key={currentQuestion.id}>
              ...
              {currentQuestion.options.map(item => (
                <button
                  ...
                  className={`p-5 rounded-2xl border text-center sm:text-start transition cursor-pointer flex items-center gap-4 ${answers[currentQuestion.id] === item.id
                      ...
                      : 'bg-[#061719] text-[#B9D0D1] border-[#0B2528] hover:border-[#0E5257]'
                    }`}
                >
                  {item.image_url ? (
            ...
            {step > 1 && (
              <div className="flex justify-between">
                <button
                  ...
                  className="text-xs text-[#8EA8AA] hover:underline cursor-pointer"
                >
                  ← {t('السابق', 'Back')}
                </button>
              </div>
        ...
        {step === questions.length + 1 && (
          <div className="space-y-6 animate-fade-in">
            <div className="p-6 rounded-3xl bg-[#0A1E20] border border-[#6CC6C9]/50 flex items-center justify-between">
              <div>
              ...
              <button
                onClick={resetQuiz}
                className="bg-[#0B2528] hover:bg-[#0E5257] text-white px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-1.5 cursor-pointer"
              >
