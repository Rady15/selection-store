...
export const OrderConfirmationPage: React.FC<OrderConfirmationPageProps> = ({ orderId, onNavigate }) => {
  ...
  if (notFound) {
    return (
      <div className="bg-[#061719] text-[#F3FBFB] min-h-screen py-20 text-center space-y-4 px-4">
        <Coffee className="w-16 h-16 text-[#0E5257] mx-auto" />
        <h2 className="text-2xl font-bold">{t('الطلب غير موجود', 'Order Not Found')}</h2>
        <p className="text-sm text-[#8EA8AA] max-w-md mx-auto">
  ...
  if (!order) {
    return (
      <div className="bg-[#061719] text-white min-h-screen py-20 flex items-center justify-center">
  ...
  return (
    <div className="bg-[#061719] text-[#F3FBFB] min-h-screen py-12">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 space-y-8">
        ...
        {/* Success Header Box */}
        <div className="p-8 rounded-3xl bg-[#0A1E20] border border-[#0B2528] text-center space-y-4 shadow-2xl">
          ...
          <p className="text-xs sm:text-sm text-[#B9D0D1] max-w-md mx-auto">
          ...
          <div className="inline-flex items-center gap-2 bg-[#061719] border border-[#0B2528] px-4 py-2 rounded-xl text-xs font-mono text-[#6CC6C9]">
            <span>{t('رقم الطلب:', 'Order Number:')}</span>
            <strong className="text-white font-bold">{order.order_number}</strong>
          </div>

          <div className="flex items-center justify-center gap-3 flex-wrap">
            <div className="inline-flex items-center gap-2 bg-[#061719] border border-[#0B2528] px-3 py-1.5 rounded-xl">
              <span className="text-[10px] text-[#8EA8AA]">{t('حالة الطلب', 'Order')}:</span>
            ...
            <div className="inline-flex items-center gap-2 bg-[#061719] border border-[#0B2528] px-3 py-1.5 rounded-xl">
              <span className="text-[10px] text-[#8EA8AA]">{t('حالة الدفع', 'Payment')}:</span>
        ...
        <div className="p-6 rounded-3xl bg-[#0A1E20] border border-[#0B2528] space-y-6">
          <div className="flex items-center justify-between border-b border-[#0B2528] pb-4">
          ...
          <div className="space-y-3">
            {order.items.map((item: any, idx) => (
              <div key={idx} className="flex justify-between items-center text-xs text-[#B9D0D1] border-b border-[#0B2528]/60 pb-3">
                <div>
                  <h5 className="font-bold text-white text-sm">
                    {language === 'ar' ? (item.product_name_ar || item.name_ar) : (item.product_name_en || item.name_en)}
                  </h5>
                  {item.weight && (
                    <p className="text-[11px] text-[#8EA8AA]">
          ...
          {/* Pricing Math */}
          <div className="space-y-1.5 text-xs text-[#B9D0D1] pt-2 border-t border-[#0B2528]">
            ...
            <div className="flex justify-between font-extrabold text-base text-white pt-2 border-t border-[#0B2528]">
        ...
        <div className="p-6 rounded-3xl bg-[#0A1E20] border border-[#0B2528] space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs text-[#B9D0D1]">
            <div className="space-y-1">
              <span className="text-[#8EA8AA] block">{t('العميل والمدينة', 'Customer & City')}</span>
            ...
            <div className="space-y-1">
              <span className="text-[#8EA8AA] block">{t('شركة الشحن وطريقة الدفع', 'Courier & Payment')}</span>
              ...
              {(order.payment_method === 'visa' || order.payment_method === 'apple_pay') && (
                <p className="text-[10px] text-[#8EA8AA] flex items-center gap-1 pt-0.5">
                  <ShieldCheck className="w-3 h-3 text-emerald-400" />
                  <span>{t('دفع آمن ومشفر عبر Stripe', 'Secure encrypted payment via Stripe')}</span>
                </p>
              )}
              {order.payment_method === 'cod' && (
                <p className="text-[10px] text-[#8EA8AA]">
              ...
              {order.status_history.map((h, idx) => (
                <div key={idx} className="flex items-start gap-3 text-xs">
                  <div className="flex flex-col items-center">
                    ...
                    {idx < order.status_history.length - 1 && <div className="w-0.5 h-8 bg-[#0B2528] mt-1" />}
                  </div>
                  <div>
                    <p className="text-white font-bold">{language === 'ar' ? h.note_ar : h.note_en}</p>
                    <span className="text-[#8EA8AA] text-[10px]">{new Date(h.timestamp).toLocaleString(language === 'ar' ? 'ar-SA' : 'en-US')}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {order.tracking_number && (
          <div className="p-4 rounded-2xl bg-[#0E5257]/10 border border-[#6CC6C9]/30 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div>
                <span className="text-[#8EA8AA]">{t('شركة الشحن', 'Carrier')}</span>
                <p className="text-white font-bold">SMSA Express</p>
              </div>
              <div>
                <span className="text-[#8EA8AA]">{t('رقم التتبع', 'Tracking Number')}</span>
        ...
        {/* Track Order Action */}
        <div className="p-4 rounded-3xl bg-[#0A1E20] border border-[#0B2528] flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2 text-xs text-[#B9D0D1]">
