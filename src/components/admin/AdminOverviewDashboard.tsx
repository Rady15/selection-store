        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="p-5 rounded-3xl bg-[#0A1E20] border border-[#0B2528] animate-pulse">
              <div className="h-4 bg-[#0B2528] rounded w-24 mb-3"></div>
              <div className="h-8 bg-[#0B2528] rounded w-32 mb-2"></div>
              <div className="h-3 bg-[#0B2528] rounded w-20"></div>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={fetchStats}
          className="p-2 rounded-xl bg-[#0A1E20] border border-[#0B2528] text-[#B9D0D1] hover:text-[#6CC6C9] transition cursor-pointer"
      ...
      {/* KPI Cards Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <div className="p-5 rounded-3xl bg-[#0A1E20] border border-[#0B2528] space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs text-[#8EA8AA] font-semibold">{t('إجمالي المبيعات', 'Total Revenue')}</span>
            <div className="p-2.5 rounded-xl bg-emerald-500/20 text-emerald-400">
              <DollarSign className="w-5 h-5" />
            </div>
          </div>
          <span className="font-extrabold text-2xl text-white block">{formatPrice(totalRevenue)}</span>
          <span className="text-[10px] text-[#8EA8AA] font-bold">
        ...
        <div className="p-5 rounded-3xl bg-[#0A1E20] border border-[#0B2528] space-y-2">
          <div className="flex items-center justify-between">
            ...
            <div className="p-2.5 rounded-xl bg-[#0E5257]/20 text-[#6CC6C9]">
          ...
          <span className="text-[10px] flex items-center gap-1.5">
            {newOrdersToday > 0 ? (
              ...
              <span className="text-[#8EA8AA] font-bold">
        ...
        <div className="p-5 rounded-3xl bg-[#0A1E20] border border-[#0B2528] space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs text-[#8EA8AA] font-semibold">{t('العملاء المسجلون', 'Total Customers')}</span>
        ...
        <div className="p-5 rounded-3xl bg-[#0A1E20] border border-[#0B2528] space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs text-[#8EA8AA] font-semibold">{t('تنبيهات المخزون', 'Low Stock Alert')}</span>
        ...
        <div className="p-6 rounded-3xl bg-[#0A1E20] border border-[#0B2528] space-y-4">
          ...
          <div className="space-y-2 text-xs">
            {recentOrders.length === 0 ? (
              <p className="text-[#8EA8AA] text-center py-4">{t('لا توجد طلبات بعد', 'No orders yet')}</p>
            ) : recentOrders.map((order: any) => (
              <div key={order.id} className="p-3 rounded-2xl bg-[#061719] border border-[#0B2528] flex justify-between items-center">
                <div>
                  <span className="font-bold text-white">{order.order_number}</span>
                  <span className="text-[#8EA8AA] mx-2">|</span>
                  <span className="text-[#B9D0D1]">{order.customer_name}</span>
                </div>
        ...
        <div className="p-6 rounded-3xl bg-[#0A1E20] border border-[#0B2528] space-y-4">
          ...
          <div className="space-y-2 text-xs">
            {lowStockProducts.length === 0 ? (
              <p className="text-[#8EA8AA] text-center py-4">{t('جميع المنتجات متوفرة', 'All products are in stock')}</p>
            ) : lowStockProducts.map((prod: any) => (
              <div key={prod.id} className="p-3 rounded-2xl bg-[#061719] border border-[#0B2528] flex justify-between items-center">
                <div>
                  <span className="font-bold text-white">{language === 'ar' ? prod.name_ar : prod.name_en}</span>
                  <span className="text-[#8EA8AA] ml-2">({prod.sku})</span>
                </div>
                <div className="flex items-center gap-2">
