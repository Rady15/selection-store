    ...
    return (
      <div className="space-y-6 animate-fade-in">
        <div>
          ...
          <p className="text-xs text-[#8EA8AA] mt-0.5">{t('إدارة مشتركي النشرة البريدية', 'Manage newsletter subscribers')}</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[1, 2, 3].map(i => (
            <div key={i} className="p-5 rounded-3xl bg-[#0A1E20] border border-[#0B2528] animate-pulse">
              <div className="h-5 bg-[#0B2528] rounded w-1/2 mb-2" />
              <div className="h-8 bg-[#0B2528] rounded w-1/3" />
            </div>
          ))}
        </div>
        <div className="space-y-2">
          {[1, 2, 3].map(i => (
            <div key={i} className="p-4 rounded-2xl bg-[#0A1E20] border border-[#0B2528] animate-pulse">
              <div className="h-4 bg-[#0B2528] rounded w-1/3" />
            </div>
          ))}
        </div>
      </div>
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="p-5 rounded-3xl bg-[#0A1E20] border border-[#0B2528]">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-[#8EA8AA]">{t('إجمالي المشتركين', 'Total Subscribers')}</span>
          </div>
        ...
        <div className="p-5 rounded-3xl bg-[#0A1E20] border border-[#0B2528]">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-[#8EA8AA]">{t('هذا الشهر', 'This Month')}</span>
          </div>
        ...
        <div className="p-5 rounded-3xl bg-[#0A1E20] border border-[#0B2528]">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-[#8EA8AA]">{t('محسوبين (منتصف الشهر)', 'Mid-Month')}</span>
          </div>
          <p className="text-2xl font-extrabold text-white">
            {subscribers.filter(s => {
              const d = new Date(s.created_at);
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-[#0A1E20] border border-[#0B2528] rounded-xl p-2.5 pl-10 text-white text-xs"
          />
        </div>
        <button
          onClick={exportCSV}
      ...
      <div className="space-y-2">
        {filtered.length === 0 ? (
          ...
          filtered.map(sub => (
            <div key={sub.id} className="p-4 rounded-2xl bg-[#0A1E20] border border-[#0B2528] flex items-center justify-between gap-3">
      ...
      {filtered.length > 0 && (
        <p className="text-center text-[10px] text-[#8EA8AA]">
