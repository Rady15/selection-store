...
export const AdminCategoriesManager: React.FC = () => {
    ...
    return (
      <div className="space-y-6 animate-fade-in">
        <div>
          ...
          <p className="text-xs text-[#8EA8AA] mt-0.5">{t('إدارة أقسام المنتجات', 'Manage product categories')}</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[1, 2, 3].map(i => (
            <div key={i} className="p-5 rounded-3xl bg-[#0A1E20] border border-[#0B2528] animate-pulse">
              <div className="w-full h-32 bg-[#0B2528] rounded-xl mb-3" />
              <div className="h-4 bg-[#0B2528] rounded w-2/3 mb-2" />
              <div className="h-3 bg-[#0B2528] rounded w-1/3" />
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {categories.map(cat => (
          <div key={cat.id} className="p-5 rounded-3xl bg-[#0A1E20] border border-[#0B2528] space-y-3">
            <div className="w-full h-32 rounded-xl overflow-hidden bg-[#0B2528] flex items-center justify-center">
              {cat.image ? (
                <img src={cat.image} alt={language === 'ar' ? cat.name_ar : cat.name_en} className="w-full h-full object-cover" />
              ) : (
                <Image className="w-8 h-8 text-[#8EA8AA]" />
              )}
            </div>
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-[10px] text-[#8EA8AA] font-mono">/{cat.slug}</p>
              </div>
              <span className="text-[9px] text-[#8EA8AA] bg-[#0B2528] px-2 py-0.5 rounded font-bold">
                #{cat.sort_order}
              </span>
            </div>
            ...
            <div className="flex justify-between items-center pt-2 border-t border-[#0B2528]">
              <button
                ...
                className={`flex items-center gap-1 text-[10px] font-bold cursor-pointer ${
                  cat.featured ? 'text-[#6CC6C9]' : 'text-[#8EA8AA]'
                }`}
                disabled
              >
              ...
              <div className="flex items-center gap-1.5">
                <button onClick={() => openEdit(cat)}
                  className="p-1.5 rounded-lg bg-[#0E5257]/20 text-[#0E5257] hover:bg-[#0E5257] hover:text-white transition cursor-pointer">
        ...
        {categories.length === 0 && (
          <div className="col-span-full p-8 text-center text-[#8EA8AA] bg-[#0A1E20] rounded-3xl border border-[#0B2528]">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto bg-[#061719] text-white border border-[#0B2528] rounded-3xl p-6 shadow-2xl z-50 space-y-4 text-xs">
            <button onClick={() => setShowModal(false)} className="absolute top-4 right-4 text-[#8EA8AA] hover:text-white cursor-pointer">
              <X className="w-5 h-5" />
            </button>
            ...
            <form onSubmit={handleSave} className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('الاسم بالعربي', 'Arabic Name')}</label>
                  <input type="text" required value={form.name_ar} onChange={e => updateField('name_ar', e.target.value)}
                    className="w-full bg-[#0A1E20] border border-[#0B2528] rounded-xl p-2.5 text-white" />
                </div>
                <div>
                  <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('الاسم بالإنجليزي', 'English Name')}</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('الوصف بالعربي', 'Arabic Description')}</label>
                  <textarea rows={2} value={form.description_ar} onChange={e => updateField('description_ar', e.target.value)}
                    className="w-full bg-[#0A1E20] border border-[#0B2528] rounded-xl p-2.5 text-white resize-none" />
                </div>
                <div>
                  <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('الوصف بالإنجليزي', 'English Description')}</label>
              ...
              <div>
                <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('الرابط المختصر', 'Slug')}</label>
                <input type="text" required value={form.slug} onChange={e => updateField('slug', e.target.value)}
                  placeholder="category-slug"
                  className="w-full bg-[#0A1E20] border border-[#0B2528] rounded-xl p-2.5 text-white font-mono" />
              </div>

              <ImageUploader value={form.image} onChange={val => updateField('image', val)} label={t('رابط الصورة', 'Image URL')} />

              <div>
                <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('الأيقونة', 'Icon')}</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('ترتيب العرض', 'Sort Order')}</label>
                ...
                <div className="flex items-end pb-1">
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    <span className="text-[#B9D0D1] font-semibold">{t('قسم مميز', 'Featured Category')}</span>
                  </label>
                </div>
              </div>
