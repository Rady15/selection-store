  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-xs text-[#8EA8AA] mt-0.5">{t('إدارة مراجعات العملاء وأسئلتهم على المنتجات', 'Manage customer reviews and product Q&A')}</p>
      </div>

      <div className="flex gap-2">
        <button onClick={() => setActiveTab('reviews')}
          className={`px-4 py-2 rounded-xl text-xs font-bold transition cursor-pointer ${activeTab === 'reviews' ? 'bg-[#0E5257] text-white' : 'bg-[#0A1E20] text-[#8EA8AA] border border-[#0B2528]'}`}>
          {t('المراجعات', 'Reviews')} ({reviews.length})
        </button>
        <button onClick={() => setActiveTab('questions')}
          className={`px-4 py-2 rounded-xl text-xs font-bold transition cursor-pointer ${activeTab === 'questions' ? 'bg-[#0E5257] text-white' : 'bg-[#0A1E20] text-[#8EA8AA] border border-[#0B2528]'}`}>
      ...
      {activeTab === 'reviews' && (
        <div className="space-y-3">
          {reviews.length === 0 ? (
            <div className="p-8 text-center text-[#8EA8AA] bg-[#0A1E20] rounded-3xl border border-[#0B2528]">
              {t('لا توجد مراجعات بعد', 'No reviews yet')}
            </div>
          ) : reviews.map(r => (
            <div key={r.id} className="p-4 rounded-2xl bg-[#0A1E20] border border-[#0B2528]">
              <div className="flex justify-between items-start">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex">
                      {[...Array(5)].map((_, i) => (
                        <Star key={i} className={`w-3 h-3 ${i < r.rating ? 'text-amber-400 fill-amber-400' : 'text-[#0B2528]'}`} />
                      ))}
                    </div>
                    {r.verified_purchase && (
                  ...
                  <p className="text-[#B9D0D1] text-xs mt-0.5">{r.comment}</p>
                  {r.staff_reply_ar && (
                    <div className="mt-2 p-2 rounded-lg bg-[#061719] border border-[#0B2528] text-xs">
                      <span className="text-[#6CC6C9] font-bold">{t('رد المحمصة:', 'Staff reply:')}</span>
                      <span className="text-[#B9D0D1] ml-1">{language === 'ar' ? r.staff_reply_ar : r.staff_reply_en}</span>
                    </div>
                  )}
                  <input
                    type="text"
                    ...
                    className="mt-2 w-full bg-[#061719] border border-[#0B2528] rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-[#6CC6C9]"
                    onKeyDown={e => {
                      if (e.key === 'Enter') {
                        e.preventDefault();
      ...
      {activeTab === 'questions' && (
        <div className="space-y-3">
          {questions.length === 0 ? (
            ...
            <div key={q.id} className="p-4 rounded-2xl bg-[#0A1E20] border border-[#0B2528]">
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  ...
                  <p className="text-[#B9D0D1] text-xs mt-1">{q.question}</p>
                  {q.answer_ar ? (
                    <div className="mt-2 p-2 rounded-lg bg-[#061719] border border-[#0B2528] text-xs">
                      <span className="text-[#6CC6C9] font-bold">{t('الإجابة:', 'Answer:')}</span>
                      <span className="text-[#B9D0D1] ml-1">{language === 'ar' ? q.answer_ar : q.answer_en}</span>
                    </div>
                  ) : (
                    <div className="mt-2">
                      <input
                        ...
                        className="w-full bg-[#061719] border border-[#0B2528] rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-[#6CC6C9]"
                        onKeyDown={e => {
                          if (e.key === 'Enter') {
                            e.preventDefault();
                            if (e.currentTarget.value.trim()) {
