    ...
    <>
      <div className="flex items-center justify-between gap-3">
        ...
        {announcement.link && announcement.show_button && (
          <span className="hidden sm:flex items-center gap-1 rounded-full bg-gradient-to-l from-[#6CC6C9] to-[#0E5257] text-[#061719] px-3 py-1 text-[10px] font-extrabold shrink-0">
    ...
    return (
      <div className="space-y-6 animate-fade-in">
        <div>
          ...
          <p className="text-xs text-[#8EA8AA] mt-0.5">{t('إدارة شريط الإعلانات العلوي', 'Manage the top announcement bar')}</p>
        </div>
        <div className="p-8 rounded-3xl bg-[#0A1E20] border border-[#0B2528] animate-pulse space-y-4">
          <div className="h-10 bg-[#0B2528] rounded-xl w-1/3" />
          <div className="h-10 bg-[#0B2528] rounded-xl w-full" />
          <div className="h-10 bg-[#0B2528] rounded-xl w-full" />
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Form */}
        <form onSubmit={handleSave} className="space-y-4 p-6 rounded-3xl bg-[#0A1E20] border border-[#0B2528]">
          <div className="flex items-center justify-between">
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              ...
              <div className="w-11 h-6 bg-[#0B2528] rounded-full peer peer-checked:bg-[#0E5257] peer-focus:ring-2 peer-focus:ring-[#6CC6C9]/30 after:content-[''] after:absolute after:top-0.5 after:right-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:-translate-x-full" />
              <span className="mr-2 text-xs font-bold text-[#B9D0D1]">
          ...
          <div>
            <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('النص بالعربي', 'Arabic Text')}</label>
            <input type="text" value={announcement.text_ar} onChange={e => updateField('text_ar', e.target.value)}
              ...
              className="w-full bg-[#061719] border border-[#0B2528] rounded-xl p-2.5 text-white" />
          </div>

          <div>
            <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('النص بالإنجليزي', 'English Text')}</label>
          ...
          <div>
            <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('الرابط', 'Link')}</label>
            <select
              value={LINK_OPTIONS.some(o => o.value === announcement.link) ? announcement.link : ''}
              onChange={e => updateField('link', e.target.value)}
              className="w-full bg-[#061719] border border-[#0B2528] rounded-xl p-2.5 text-white cursor-pointer"
            >
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('لون الخلفية', 'Background Color')}</label>
              <div className="flex items-center gap-2">
                <input type="color" value={announcement.bg_color} onChange={e => updateField('bg_color', e.target.value)}
                  className="w-10 h-10 rounded-lg border border-[#0B2528] cursor-pointer bg-transparent" />
                <input type="text" value={announcement.bg_color} onChange={e => updateField('bg_color', e.target.value)}
                  className="flex-1 bg-[#061719] border border-[#0B2528] rounded-xl p-2.5 text-white font-mono text-xs" />
              </div>
            </div>
            <div>
              <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('لون النص', 'Text Color')}</label>
              <div className="flex items-center gap-2">
          ...
          <div>
            <label className="block text-[#B9D0D1] mb-1 font-semibold">
          ...
          <div className="pt-1 border-t border-[#0B2528]/60">
            <p className="text-[11px] font-bold text-[#6CC6C9] mb-3">{t('خيارات العرض المميز (اختياري)', 'Featured Offer Options (optional)')}</p>
          </div>
          ...
          <div>
            ...
            <p className="text-[10px] text-[#6CC6C9] mt-1">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('أوفرلاي داكن', 'Dark Overlay')}</label>
              <label className="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" checked={announcement.overlay}
                  onChange={e => updateField('overlay', e.target.checked)} className="sr-only peer" />
                <div className="w-10 h-5.5 bg-[#0B2528] rounded-full peer peer-checked:bg-[#0E5257] peer-focus:ring-2 peer-focus:ring-[#6CC6C9]/30 after:content-[''] after:absolute after:top-0.5 after:right-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:after:-translate-x-full" />
                <span className="mr-2 text-xs font-bold text-[#B9D0D1]">
            ...
            <div>
              <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('إظهار الزر', 'Show Button')}</label>
              <label className="relative inline-flex items-center cursor-pointer">
            ...
            <div>
              <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('الإطار الذهبي', 'Gold Frame')}</label>
              <label className="relative inline-flex items-center cursor-pointer">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('نص الزر عربي', 'CTA Text (AR)')}</label>
              <input type="text" value={announcement.cta_text_ar} onChange={e => updateField('cta_text_ar', e.target.value)}
                placeholder="تسوق الآن"
                className="w-full bg-[#061719] border border-[#0B2528] rounded-xl p-2.5 text-white" />
            </div>
            <div>
              <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('نص الزر إنجليزي', 'CTA Text (EN)')}</label>
        ...
        <div className="space-y-4">
          <div className="p-6 rounded-3xl bg-[#0A1E20] border border-[#0B2528]">
            <h3 className="font-bold text-sm text-white mb-4 flex items-center gap-2">
              <Eye className="w-4 h-4 text-[#6CC6C9]" />
              {t('معاينة', 'Preview')}
            </h3>
            ...
            {announcement.show_frame === false ? (
              ...
              <div className="rounded-2xl bg-gradient-to-l from-[#6CC6C9]/60 via-[#0E5257]/25 to-[#6CC6C9]/60 p-px">
            ...
            <div className="mt-4 space-y-2 text-xs text-[#8EA8AA]">
          ...
          <div className="p-6 rounded-3xl bg-[#0A1E20] border border-[#0B2528]">
            <h3 className="font-bold text-sm text-white mb-3 flex items-center gap-2">
              <Megaphone className="w-4 h-4 text-[#6CC6C9]" />
              {t('معاينة الموقع الكامل', 'Full Site Preview')}
            </h3>
            <div className="rounded-xl overflow-hidden border border-[#0B2528]">
              {announcement.show_frame === false ? (
                ...
                <div className="rounded-lg bg-gradient-to-l from-[#6CC6C9]/50 via-[#0E5257]/20 to-[#6CC6C9]/50 p-px m-1">
              ...
              <div className="h-6 bg-[#0B2528] flex items-center justify-center">
                <div className="w-8 h-1 bg-[#8EA8AA]/30 rounded" />
              </div>
              <div className="bg-[#061719] p-3 space-y-2">
                <div className="h-2 bg-[#0B2528] rounded w-3/4" />
                <div className="h-2 bg-[#0B2528] rounded w-1/2" />
              </div>
            </div>
          </div>
        </div>
