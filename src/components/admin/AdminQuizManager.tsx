...
export const AdminQuizManager: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="bg-[#0B2528] hover:bg-[#18373A] text-white px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-1.5 cursor-pointer border border-[#18373A]"
          >
      ...
      {/* Settings Section */}
      <div className="p-5 rounded-3xl bg-[#0A1E20] border border-[#0B2528] space-y-4">
        <h3 className="text-sm font-extrabold text-[#6CC6C9] border-b border-[#0B2528] pb-2">
          {t('إعدادات الاختبار', 'Quiz Settings')}
        </h3>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            <label className="text-[10px] font-bold text-[#8EA8AA] block mb-1">{t('النتيجة الأساسية', 'Base Score')}</label>
            <input type="number" className="bg-[#061719] border border-[#0B2528] rounded-lg px-3 py-2 text-xs text-white w-full"
              value={config.settings.base_score}
              onChange={e => updateSettings('base_score', parseInt(e.target.value) || 0)}
            />
          </div>
          <div>
            <label className="text-[10px] font-bold text-[#8EA8AA] block mb-1">{t('عدد النتائج', 'Results Count')}</label>
            <input type="number" className="bg-[#061719] border border-[#0B2528] rounded-lg px-3 py-2 text-xs text-white w-full"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            <label className="text-[10px] font-bold text-[#8EA8AA] block mb-1">{t('الشارة (عربي)', 'Badge (AR)')}</label>
            <input className="bg-[#061719] border border-[#0B2528] rounded-lg px-3 py-2 text-xs text-white w-full" dir="rtl"
              value={config.settings.badge_ar}
              onChange={e => updateSettings('badge_ar', e.target.value)}
            />
          </div>
          <div>
            <label className="text-[10px] font-bold text-[#8EA8AA] block mb-1">{t('الشارة (إنجليزي)', 'Badge (EN)')}</label>
            <input className="bg-[#061719] border border-[#0B2528] rounded-lg px-3 py-2 text-xs text-white w-full"
              value={config.settings.badge_en}
              onChange={e => updateSettings('badge_en', e.target.value)}
            />
          </div>
          <div>
            <label className="text-[10px] font-bold text-[#8EA8AA] block mb-1">{t('العنوان (عربي)', 'Title (AR)')}</label>
            <input className="bg-[#061719] border border-[#0B2528] rounded-lg px-3 py-2 text-xs text-white w-full" dir="rtl"
          ...
          <div>
            <label className="text-[10px] font-bold text-[#8EA8AA] block mb-1">{t('العنوان (إنجليزي)', 'Title (EN)')}</label>
            <input className="bg-[#061719] border border-[#0B2528] rounded-lg px-3 py-2 text-xs text-white w-full"
          ...
          <div className="sm:col-span-2">
            <label className="text-[10px] font-bold text-[#8EA8AA] block mb-1">{t('الوصف (عربي)', 'Subtitle (AR)')}</label>
            <input className="bg-[#061719] border border-[#0B2528] rounded-lg px-3 py-2 text-xs text-white w-full" dir="rtl"
          ...
          <div className="sm:col-span-2">
            <label className="text-[10px] font-bold text-[#8EA8AA] block mb-1">{t('الوصف (إنجليزي)', 'Subtitle (EN)')}</label>
            <input className="bg-[#061719] border border-[#0B2528] rounded-lg px-3 py-2 text-xs text-white w-full"
      ...
      {config.questions.length === 0 && (
        <div className="p-8 rounded-3xl bg-[#0A1E20] border border-[#0B2528] text-center text-[#8EA8AA] text-sm">
          {t('لا توجد أسئلة بعد. أضف سؤالاً للبدء.', 'No questions yet. Add a question to start.')}
        </div>
      )}

      {config.questions.map((q, qi) => (
        <div key={q.id} className="p-5 rounded-3xl bg-[#0A1E20] border border-[#0B2528] space-y-4">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2 flex-1">
              <div className="flex flex-col gap-0.5">
                <button onClick={() => moveQuestion(qi, -1)} className="p-0.5 rounded hover:bg-[#0B2528] text-[#8EA8AA] hover:text-white transition cursor-pointer disabled:opacity-30" disabled={qi === 0}>
                  <ArrowUp className="w-3 h-3" />
                </button>
                <button onClick={() => moveQuestion(qi, 1)} className="p-0.5 rounded hover:bg-[#0B2528] text-[#8EA8AA] hover:text-white transition cursor-pointer disabled:opacity-30" disabled={qi === config.questions.length - 1}>
                  <ArrowDown className="w-3 h-3" />
                </button>
              </div>
              <span className="text-[10px] font-bold text-[#6CC6C9] bg-[#0B2528] px-2 py-0.5 rounded flex-shrink-0">
                Q{qi + 1}
              </span>
              <input
                className="bg-[#061719] border border-[#0B2528] rounded-lg px-3 py-1.5 text-xs text-white w-full max-w-[200px]"
            ...
            <div className="flex items-center gap-2">
              <label className="flex items-center gap-1.5 text-[10px] text-[#8EA8AA] cursor-pointer">
                <input
                  type="checkbox"
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-bold text-[#8EA8AA] uppercase tracking-wider">
                {t('الخيارات', 'Options')}
              </span>
            ...
            {q.options.map((o, oi) => (
              <div key={o.id} className="p-3 rounded-2xl bg-[#061719] border border-[#0B2528] space-y-2">
                <div className="flex items-center gap-2 flex-wrap">
                  <div className="flex flex-col gap-0.5">
                    <button onClick={() => moveOption(qi, oi, -1)} className="p-0.5 rounded hover:bg-[#0A1E20] text-[#8EA8AA] hover:text-white transition cursor-pointer disabled:opacity-30" disabled={oi === 0}>
                      <ArrowUp className="w-2.5 h-2.5" />
                    </button>
                    <button onClick={() => moveOption(qi, oi, 1)} className="p-0.5 rounded hover:bg-[#0A1E20] text-[#8EA8AA] hover:text-white transition cursor-pointer disabled:opacity-30" disabled={oi === q.options.length - 1}>
                      <ArrowDown className="w-2.5 h-2.5" />
                    </button>
                  </div>
                  <input
                    className="bg-[#0A1E20] border border-[#0B2528] rounded-lg px-2 py-1 text-[11px] text-white w-10 text-center"
                    placeholder="icon"
                    value={o.icon}
                  ...
                  <div className="relative flex-1 min-w-[100px]">
                    <input
                      className="bg-[#0A1E20] border border-[#0B2528] rounded-lg px-2 py-1 text-[11px] text-white w-full pl-7"
                      placeholder={t('رابط الصورة (اختياري)', 'Image URL (optional)')}
                      value={o.image_url || ''}
                      onChange={e => updateOption(qi, oi, 'image_url', e.target.value)}
                    />
                    <ImageIcon className="w-3.5 h-3.5 text-[#8EA8AA] absolute left-2 top-1/2 -translate-y-1/2 pointer-events-none" />
                  </div>
                  <input
                    className="bg-[#0A1E20] border border-[#0B2528] rounded-lg px-2 py-1 text-[11px] text-white flex-1 min-w-[120px]"
                    placeholder="العربية"
                    value={o.label_ar}
                ...
                <div className="pl-3 border-l-2 border-[#0B2528] space-y-1.5">
                  <div className="flex items-center justify-between">
                    <span className="text-[9px] font-bold text-[#8EA8AA] uppercase tracking-wider">
                  ...
                  {o.score_rules.map((r, ri) => (
                    <div key={ri} className="flex items-center gap-1.5 flex-wrap">
                      <select
                        className="bg-[#0A1E20] border border-[#0B2528] rounded-lg px-2 py-1 text-[10px] text-white"
                        value={r.field}
                      ...
                      <input
                        className="bg-[#0A1E20] border border-[#0B2528] rounded-lg px-2 py-1 text-[10px] text-white w-28"
                      ...
                      <div className="flex items-center gap-1">
                        <input
                          type="number"
                          className="bg-[#0A1E20] border border-[#0B2528] rounded-lg px-2 py-1 text-[10px] text-white w-14"
                          value={r.points}
                          onChange={e => updateRule(qi, oi, ri, 'points', parseInt(e.target.value) || 0)}
                        />
                        <span className="text-[9px] text-[#8EA8AA]">{t('نقطة', 'pts')}</span>
                      </div>
                  ...
                  {o.score_rules.length === 0 && (
                    <p className="text-[9px] text-[#8EA8AA] italic">
