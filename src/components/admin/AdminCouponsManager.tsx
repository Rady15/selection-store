    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-[#8EA8AA] mt-0.5">{t('إدارة وتوليد كوبونات الخصم الترويجية', 'Manage promo discount codes')}</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {coupons.map(c => (
          <div key={c.id} className="p-5 rounded-3xl bg-[#0A1E20] border border-[#0B2528] space-y-3">
            <div className="flex justify-between items-center">
              <span className="font-extrabold text-lg text-[#6CC6C9] font-mono tracking-wider">{c.code}</span>
            ...
            <div className="flex justify-between items-center pt-2 border-t border-[#0B2528] text-xs">
              <span className="text-[#8EA8AA]">{t('الاستخدام:', 'Used:')} {c.usage_count}/{c.usage_limit}</span>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-[#061719] text-white border border-[#0B2528] rounded-3xl p-6 shadow-2xl z-50 space-y-4 text-xs">
            ...
            <form onSubmit={handleCreateCoupon} className="space-y-3">
              <div>
                <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('كود الخصم', 'Coupon Code')}</label>
                <input type="text" required value={code} onChange={e => setCode(e.target.value)}
                  placeholder="WELCOME10"
                  className="w-full bg-[#0A1E20] border border-[#0B2528] rounded-xl p-2.5 text-white uppercase font-mono" />
              </div>

              <div>
                <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('نوع الخصم', 'Discount Type')}</label>
                  ...
                  {(['percentage', 'fixed', 'free_shipping'] as const).map(type => (
                    <button key={type} type="button"
                      ...
                      className={`p-2 rounded-xl border text-center font-bold transition cursor-pointer ${
                        discountType === type ? 'bg-[#0E5257] border-[#0E5257] text-white' : 'bg-[#0A1E20] border-[#0B2528] text-[#B9D0D1]'
                      }`}>
              ...
              {discountType === 'percentage' && (
                <div>
                  <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('حد أقصى للخصم (﷼)', 'Max Discount (﷼)')}</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('الحد الأدنى للطلب (﷼)', 'Min Order (SAR)')}</label>
                  <input type="number" required value={minOrderAmount} onChange={e => setMinOrderAmount(Number(e.target.value))}
                    className="w-full bg-[#0A1E20] border border-[#0B2528] rounded-xl p-2.5 text-white" />
                </div>
                <div>
                  <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('حد الاستخدام', 'Usage Limit')}</label>
              ...
              <div>
                <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('صالح حتى', 'Valid Until')}</label>
                <input type="date" required value={validUntil} onChange={e => setValidUntil(e.target.value)}
                  className="w-full bg-[#0A1E20] border border-[#0B2528] rounded-xl p-2.5 text-white" />
              </div>

              <button type="submit" className="w-full bg-[#0E5257] hover:bg-[#2B7D82] text-white py-3 rounded-xl font-bold cursor-pointer transition">
