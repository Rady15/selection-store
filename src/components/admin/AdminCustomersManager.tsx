    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-xs text-[#8EA8AA] mt-0.5">{t('عرض وإدارة جميع المستخدمين', 'View and manage all users')}</p>
        </div>
        <div className="relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#8EA8AA]" />
          <input
            type="text"
            value={searchTerm}
            onChange={e => setSearchTerm(e.target.value)}
            placeholder={t('بحث بالاسم أو البريد...', 'Search by name or email...')}
            className="w-64 bg-[#0A1E20] border border-[#0B2528] rounded-xl py-2.5 pl-10 pr-4 text-xs text-white placeholder-[#8EA8AA] focus:outline-none focus:border-[#6CC6C9] transition"
          />
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <div className="p-5 rounded-3xl bg-[#0A1E20] border border-[#0B2528]">
          <div className="flex items-center justify-between">
            <span className="text-xs text-[#8EA8AA] font-semibold">{t('العملاء', 'Customers')}</span>
            <Users className="w-5 h-5 text-[#6CC6C9]" />
          </div>
          <span className="font-extrabold text-2xl text-white block mt-2">{customers.length}</span>
        </div>
        <div className="p-5 rounded-3xl bg-[#0A1E20] border border-[#0B2528]">
          <div className="flex items-center justify-between">
            <span className="text-xs text-[#8EA8AA] font-semibold">{t('المديرين', 'Admins')}</span>
        ...
        <div className="p-5 rounded-3xl bg-[#0A1E20] border border-[#0B2528]">
          <div className="flex items-center justify-between">
            <span className="text-xs text-[#8EA8AA] font-semibold">{t('إجمالي النقاط', 'Total Points')}</span>
        ...
        <div className="p-5 rounded-3xl bg-[#0A1E20] border border-[#0B2528]">
          <div className="flex items-center justify-between">
            <span className="text-xs text-[#8EA8AA] font-semibold">{t('إجمالي الطلبات', 'Total Orders')}</span>
      ...
      <div className="bg-[#0A1E20] border border-[#0B2528] rounded-3xl overflow-hidden shadow-2xl">
        <div className="overflow-x-auto">
          <table className="w-full text-xs text-start">
            <thead className="bg-[#061719] text-[#8EA8AA] uppercase font-bold border-b border-[#0B2528]">
              <tr>
            ...
            <tbody className="divide-y divide-[#0B2528]/60 text-[#B9D0D1]">
              {filteredUsers.map(u => (
                <tr key={u.id} className="hover:bg-[#061719]/50 transition">
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${
                        u.role === 'admin' ? 'bg-[#0E5257] text-white' : 'bg-[#0B2528] text-[#6CC6C9]'
                      }`}>
                        {(u.name || u.email || '?').charAt(0)}
                      </div>
                      <div>
                        <span className="font-bold text-white block">{u.name || u.email || '-'}</span>
                        <span className="text-[10px] text-[#8EA8AA]">{u.id}</span>
                      </div>
                    </div>
                  </td>
                  <td className="p-4">
                    <p className="flex items-center gap-1 text-[#B9D0D1]"><Mail className="w-3 h-3 text-[#8EA8AA]" />{u.email}</p>
                    {u.phone && <p className="flex items-center gap-1 text-[#B9D0D1] mt-0.5"><Phone className="w-3 h-3 text-[#8EA8AA]" />{u.phone}</p>}
                  </td>
                  <td className="p-4">
                  ...
                  <td className="p-4">
                    <span className="text-[#B9D0D1]">{u.addresses?.length || 0}</span>
                  </td>
                  <td className="p-4 text-[#8EA8AA] text-[10px]">
                    {new Date(u.created_at).toLocaleDateString(language === 'ar' ? 'ar-SA' : 'en-US')}
                  </td>
                  <td className="p-4">
                    <button
                      onClick={() => openUserDetail(u)}
                      className="flex items-center gap-1 text-[#6CC6C9] hover:text-[#F3FBFB] transition text-[11px] font-bold cursor-pointer"
                    >
      ...
      {selectedUser && detailData && (
        <div className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-12 overflow-y-auto">
          ...
          <div className="relative w-full max-w-4xl bg-[#061719] text-white border border-[#0B2528] rounded-3xl shadow-2xl z-50 animate-fade-in overflow-hidden">
            <div className="sticky top-0 bg-[#061719] z-10 p-4 sm:p-6 border-b border-[#0B2528] flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold ${
                  detailData.user.role === 'admin' ? 'bg-[#0E5257] text-white' : 'bg-[#0B2528] text-[#6CC6C9]'
                }`}>
                ...
                <div>
                  ...
                  <p className="text-[10px] text-[#8EA8AA]">{detailData.user.email} · {detailData.user.id}</p>
                </div>
              </div>
              <button onClick={closeDetail} className="p-1.5 rounded-lg text-[#8EA8AA] hover:text-white hover:bg-[#0A1E20] transition cursor-pointer">
                <X className="w-5 h-5" />
              </button>
            </div>

            <div className="flex border-b border-[#0B2528] overflow-x-auto">
              {[
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2 px-4 py-3 text-xs font-bold border-b-2 transition cursor-pointer shrink-0 ${
                      active ? 'border-[#6CC6C9] text-[#6CC6C9]' : 'border-transparent text-[#8EA8AA] hover:text-white'
                    }`}
                  >
            ...
            <div className="p-4 sm:p-6 max-h-[60vh] overflow-y-auto space-y-4">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {statItems.map(item => {
                      ...
                      return (
                        <div key={item.key} className={`${item.bg} rounded-2xl p-4 border border-[#0B2528]`}>
                          <Icon className={`w-4 h-4 ${item.color} mb-1`} />
                          <p className="text-lg font-extrabold text-white">
                            {item.key === 'total_spent' ? `${val.toLocaleString()} ${t('ريال', 'SAR')}` : val.toLocaleString()}
                          </p>
                          <p className="text-[10px] text-[#8EA8AA]">{label}</p>
                        </div>
                      );
                    })}
                  </div>

                  <div className="bg-[#0A1E20] rounded-2xl p-4 border border-[#0B2528] space-y-3">
                    <div className="flex items-center justify-between">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                      <div>
                        <span className="text-[#8EA8AA]">{t('الاسم', 'Name')}</span>
                        {editing ? (
                          <input value={editForm.name} onChange={e => setEditForm({ ...editForm, name: e.target.value })}
                            className="w-full bg-[#061719] border border-[#0B2528] rounded-lg p-2 text-white mt-1 focus:outline-none focus:border-[#6CC6C9]" />
                        ) : <p className="font-bold text-white mt-0.5">{detailData.user.name}</p>}
                      </div>
                      <div>
                        <span className="text-[#8EA8AA]">{t('البريد', 'Email')}</span>
                        {editing ? (
                      ...
                      <div>
                        <span className="text-[#8EA8AA]">{t('الجوال', 'Phone')}</span>
                        {editing ? (
                      ...
                      <div>
                        <span className="text-[#8EA8AA]">{t('الدور', 'Role')}</span>
                        {editing ? (
                      ...
                      <div>
                        <span className="text-[#8EA8AA]">{t('تاريخ التسجيل', 'Registered')}</span>
                        <p className="font-bold text-white mt-0.5">
                          {new Date(detailData.user.created_at).toLocaleDateString(language === 'ar' ? 'ar-SA' : 'en-US')}
                        </p>
                      </div>
                      <div>
                        <span className="text-[#8EA8AA]">{t('آخر طلب', 'Last Order')}</span>
                        <p className="font-bold text-white mt-0.5">
              ...
              {detailTab === 'orders' && (
                <div className="space-y-2">
                  {detailData.orders.length === 0 ? (
                    <p className="text-center text-[#8EA8AA] text-xs py-8">{t('لا توجد طلبات', 'No orders yet')}</p>
                  ) : (
                    detailData.orders.map(order => (
                      <div key={order.id} className="bg-[#0A1E20] rounded-2xl p-4 border border-[#0B2528] flex items-center justify-between flex-wrap gap-2">
                        <div>
                          <span className="font-bold text-white text-sm">{order.order_number}</span>
                          <p className="text-[10px] text-[#8EA8AA]">{new Date(order.created_at).toLocaleDateString()}</p>
                        </div>
              ...
              {detailTab === 'reviews' && (
                <div className="space-y-2">
                  {detailData.reviews.length === 0 ? (
                    <p className="text-center text-[#8EA8AA] text-xs py-8">{t('لا توجد تقييمات', 'No reviews yet')}</p>
                  ) : (
                    detailData.reviews.map(review => (
                      <div key={review.id} className="bg-[#0A1E20] rounded-2xl p-4 border border-[#0B2528]">
                        <div className="flex items-center gap-2 mb-1">
                          <div className="flex text-amber-400">
                            {Array.from({ length: review.rating }, (_, i) => <Star key={i} className="w-3 h-3 fill-current" />)}
                            {Array.from({ length: 5 - review.rating }, (_, i) => <Star key={i} className="w-3 h-3 text-[#0B2528]" />)}
                          </div>
                        ...
                        <p className="text-xs text-[#B9D0D1]">{review.comment}</p>
                        <p className="text-[10px] text-[#8EA8AA] mt-1">{new Date(review.created_at).toLocaleDateString()}</p>
                      </div>
                    ))
                  )}
                </div>
              ...
              {detailTab === 'addresses' && (
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <p className="text-xs text-[#8EA8AA]">{t('عناوين هذا المستخدم المستخدمة في الشحن والفوترة', 'Shipping & billing addresses for this user')}</p>
                    <button
                  ...
                  {addressFormOpen && (
                    <div className="bg-[#0A1E20] rounded-2xl p-4 border border-[#6CC6C9]/40 space-y-3">
                      <h3 className="font-bold text-sm">{editingAddress ? t('تعديل العنوان', 'Edit Address') : t('عنوان جديد', 'New Address')}</h3>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                        <div>
                          <span className="text-[#8EA8AA]">{t('عنوان الحفظ', 'Label')}</span>
                        ...
                        <div>
                          <span className="text-[#8EA8AA]">{t('الاسم الكامل', 'Full Name')}</span>
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="text-[#8EA8AA] hover:text-white px-3 py-2 rounded-xl text-[11px] font-bold transition cursor-pointer"
                        >
                          {t('إلغاء', 'Cancel')}
                        </button>
                      </div>
                  ...
                  {detailData.addresses.length === 0 ? (
                    <p className="text-center text-[#8EA8AA] text-xs py-8">{t('لا توجد عناوين', 'No addresses saved')}</p>
                  ) : (
                    detailData.addresses.map(addr => (
                      <div key={addr.id} className="bg-[#0A1E20] rounded-2xl p-4 border border-[#0B2528]">
                          ...
                          <div className="ms-auto flex items-center gap-1">
                            {!addr.is_default && (
                              <button
                                ...
                                className="p-1.5 rounded-lg text-[#8EA8AA] hover:text-emerald-400 hover:bg-emerald-500/10 transition cursor-pointer"
                              >
                            ...
                            <button
                              ...
                              title={t('تعديل', 'Edit')}
                              className="p-1.5 rounded-lg text-[#8EA8AA] hover:text-[#6CC6C9] hover:bg-[#6CC6C9]/10 transition cursor-pointer"
                            >
                            ...
                            <button
                              ...
                              title={t('حذف', 'Delete')}
                              className="p-1.5 rounded-lg text-[#8EA8AA] hover:text-red-400 hover:bg-red-500/10 transition cursor-pointer"
                            >
                              <Trash2 className="w-3.5 h-3.5" />
                            </button>
                          </div>
                        </div>
                        <p className="text-xs text-[#B9D0D1]">{addr.street}, {addr.district}, {addr.city}</p>
                        <p className="text-[10px] text-[#8EA8AA]">{addr.full_name} · {addr.phone}</p>
                      </div>
                    ))
                  )}
                </div>
              ...
              {detailTab === 'loyalty' && (
                <div className="space-y-4">
                  <div className="bg-[#0A1E20] rounded-2xl p-4 border border-[#0B2528]">
                    <p className="text-[#8EA8AA] text-xs">{t('الرصيد الحالي', 'Current Balance')}</p>
                  ...
                  <div className="bg-[#0A1E20] rounded-2xl p-4 border border-[#0B2528] space-y-3">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className={`flex-1 py-2 rounded-xl text-xs font-bold cursor-pointer transition ${
                          loyaltyType === 'add' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-[#061719] text-[#8EA8AA] border border-[#0B2528]'
                        }`}
                      >
                      ...
                      <button
                        ...
                        className={`flex-1 py-2 rounded-xl text-xs font-bold cursor-pointer transition ${
                          loyaltyType === 'deduct' ? 'bg-red-500/20 text-red-400 border border-red-500/30' : 'bg-[#061719] text-[#8EA8AA] border border-[#0B2528]'
                        }`}
                      >
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        className="w-28 bg-[#061719] border border-[#0B2528] rounded-xl p-2.5 text-xs text-white placeholder-[#8EA8AA] focus:outline-none focus:border-[#6CC6C9]"
                      />
                      <input
                        type="text"
                        value={loyaltyReason}
                        ...
                        className="flex-1 bg-[#061719] border border-[#0B2528] rounded-xl p-2.5 text-xs text-white placeholder-[#8EA8AA] focus:outline-none focus:border-[#6CC6C9]"
                      />
                      <button
                  ...
                  <div className="space-y-1.5">
                    <h3 className="font-bold text-xs text-[#8EA8AA]">{t('سجل النقاط', 'Transaction History')}</h3>
                    {loyaltyTransactions.length === 0 ? (
                      <p className="text-center text-[#8EA8AA] text-xs py-4">{t('لا توجد معاملات', 'No transactions yet')}</p>
                    ) : (
                      loyaltyTransactions.map(tx => (
                        <div key={tx.id} className="bg-[#0A1E20] rounded-xl p-3 border border-[#0B2528] flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-[#8EA8AA]">{new Date(tx.created_at).toLocaleDateString()}</p>
                            </div>
                          </div>
