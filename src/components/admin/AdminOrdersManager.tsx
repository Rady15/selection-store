    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="relative max-w-sm">
        <input
          ...
          className="w-full bg-[#0A1E20] border border-[#0B2528] rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-[#6CC6C9]"
        />
      </div>
        ...
        {['all', 'pending', 'paid', 'roasting', 'shipped', 'delivered', 'cancelled'].map(s => (
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-xs font-bold transition cursor-pointer ${
              statusFilter === s
                ? 'bg-[#6CC6C9] text-[#061719]'
                : 'bg-[#0A1E20] text-[#8EA8AA] border border-[#0B2528] hover:border-[#6CC6C9]'
            }`}
          >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs text-start">
            ...
            <tbody className="divide-y divide-[#0B2528]/60 text-[#B9D0D1]">
              {filtered.length === 0 ? (
                <tr><td colSpan={6} className="p-8 text-center text-[#8EA8AA]">{t('لا توجد طلبات', 'No orders found')}</td></tr>
              ) : filtered.map(ord => (
                <tr key={ord.id} className="hover:bg-[#061719]/50 transition">
                  <td className="p-4">
                    <span className="font-extrabold text-white block text-sm">{ord.order_number}</span>
                    <span className="text-[10px] text-[#8EA8AA]">{ord.customer_name} · {ord.phone}</span>
                  </td>
                  <td className="p-4">
                  ...
                  <td className="p-4">
                    <select
                      ...
                      className={`bg-[#061719] text-white border border-[#0B2528] rounded-xl px-2.5 py-1.5 text-xs focus:outline-none focus:border-[#6CC6C9] cursor-pointer disabled:opacity-60 ${getStatusColor(getEffectiveStatus(ord))}`}
                    >
                      {statusOptions.map(s => (
                  ...
                  <td className="p-4 text-end">
                    <div className="flex items-center justify-end gap-1.5">
                      <button
                        ...
                        className="p-1.5 rounded-lg bg-[#0B2528] text-white hover:bg-[#0E5257] transition cursor-pointer"
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-lg bg-[#0B2528] text-red-400 hover:bg-red-500/20 transition cursor-pointer disabled:opacity-50"
      ...
      {selectedOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-[#061719] text-white border border-[#0B2528] rounded-3xl p-6 shadow-2xl z-50 max-h-[90vh] overflow-y-auto">
            <button onClick={() => setSelectedOrder(null)} className="absolute top-4 right-4 text-[#8EA8AA] hover:text-white cursor-pointer">
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="flex items-center gap-2 mb-4">
              <button
                ...
                className="flex items-center gap-1.5 bg-[#6CC6C9] text-[#061719] px-3 py-1.5 rounded-xl text-xs font-bold hover:bg-[#7BD4D7] transition cursor-pointer"
              >
            ...
            <div className="space-y-3 text-xs">
              <div className="p-3 rounded-2xl bg-[#0A1E20] border border-[#0B2528]">
                <span className="text-[#8EA8AA] font-semibold">{t('العميل', 'Customer')}</span>
                <p className="text-white font-bold mt-1">{selectedOrder.customer_name} · {selectedOrder.phone}</p>
                <p className="text-[#B9D0D1]">{selectedOrder.email}</p>
              </div>

              <div className="p-3 rounded-2xl bg-[#0A1E20] border border-[#0B2528]">
                <span className="text-[#8EA8AA] font-semibold">{t('عنوان الشحن', 'Shipping Address')}</span>
                {selectedOrder.shipping_address ? (
                  <>
                    ...
                    <p className="text-[#B9D0D1]">{selectedOrder.shipping_address.street} {selectedOrder.shipping_address.building}</p>
                  </>
                ) : (
                  <p className="text-[#8EA8AA] mt-1">—</p>
                )}
              </div>

              <div className="p-3 rounded-2xl bg-[#0A1E20] border border-[#0B2528]">
                <span className="text-[#8EA8AA] font-semibold">{t('المنتجات', 'Items')}</span>
                {selectedOrder.items.map((item: any, idx) => (
                  <div key={idx} className="flex justify-between mt-2 text-[#B9D0D1]">
                    <div>
                      <span>{language === 'ar' ? (item.product_name_ar || item.name_ar) : (item.product_name_en || item.name_en)} × {item.quantity}</span>
                      {item.weight && (
                        <span className="text-[10px] text-[#8EA8AA] block">{item.weight}{item.grind ? ` • ${item.grind}` : ''}</span>
                      )}
                    </div>
              ...
              <div className="p-3 rounded-2xl bg-[#0A1E20] border border-[#0B2528] space-y-1">
                <div className="flex justify-between text-[#B9D0D1]">
                ...
                <div className="flex justify-between text-[#6CC6C9] font-extrabold text-sm border-t border-[#0B2528] pt-2">
              ...
              {selectedOrder.tracking_number && (
                <div className="p-3 rounded-2xl bg-[#0A1E20] border border-[#0B2528]">
                  <span className="text-[#8EA8AA] font-semibold">{t('رقم التتبع', 'Tracking')}</span>
              ...
              {selectedOrder.customer_notes && (
                <div className="p-3 rounded-2xl bg-[#0A1E20] border border-[#0B2528]">
                  <span className="text-[#8EA8AA] font-semibold">{t('ملاحظات العميل', 'Customer Notes')}</span>
                  <p className="text-[#B9D0D1] mt-1">{selectedOrder.customer_notes}</p>
                </div>
              )}

              {selectedOrder.status_history.length > 0 && (
                <div className="p-3 rounded-2xl bg-[#0A1E20] border border-[#0B2528]">
                  <span className="text-[#8EA8AA] font-semibold">{t('سجل الحالات', 'Status History')}</span>
                    ...
                    {selectedOrder.status_history.map((h, idx) => (
                      <div key={idx} className="flex items-start gap-2 text-xs">
                        ...
                        <div>
                          ...
                          <span className="text-[#8EA8AA] block">{new Date(h.timestamp).toLocaleDateString(language === 'ar' ? 'ar-SA' : 'en-US')}</span>
                        </div>
                      </div>
                    ))}
                  </div>
