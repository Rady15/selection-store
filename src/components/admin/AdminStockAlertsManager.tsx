        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs text-start">
            ...
            <tbody className="divide-y divide-[#0B2528]/60 text-[#B9D0D1]">
              {alerts.length === 0 ? (
                <tr><td colSpan={5} className="p-8 text-center text-[#8EA8AA]">
                  {t('لا توجد طلبات تنبيه', 'No alert subscriptions yet')}
                </td></tr>
              ) : alerts.map(a => (
                <tr key={a.id} className="hover:bg-[#061719]/50 transition">
                  <td className="p-4">
                    <span className="font-bold text-white block">{a.customer_name}</span>
                    <span className="text-[10px] text-[#8EA8AA]">{a.phone} &middot; {a.email}</span>
                  </td>
                  <td className="p-4 font-bold text-[#6CC6C9]">
                    {language === 'ar' ? a.product_name_ar : a.product_name_en}
                    <span className="block text-[10px] text-[#8EA8AA] font-normal">{a.variant_info}</span>
                  </td>
                  <td className="p-4 text-[#8EA8AA]">{formatDate(a.created_at)}</td>
                  <td className="p-4">
