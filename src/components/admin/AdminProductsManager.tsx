    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="bg-[#0E5257] hover:bg-[#2B7D82] text-white px-5 py-2.5 rounded-xl text-xs font-bold transition shadow-lg cursor-pointer flex items-center gap-1.5"
        >
      ...
      <div className="relative max-w-sm">
        ...
        <input
          ...
          className="w-full bg-[#0A1E20] border border-[#0B2528] rounded-xl pl-9 pr-3 py-2 text-xs text-white focus:outline-none focus:border-[#6CC6C9]"
        />
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs text-start">
            ...
            <tbody className="divide-y divide-[#0B2528]/60 text-[#B9D0D1]">
              {filtered.map(p => (
                <tr key={p.id} className="hover:bg-[#061719]/50 transition">
                  <td className="p-4 flex items-center gap-3">
                    <img src={p.images[0]} alt="" className="w-10 h-10 rounded-xl object-cover bg-[#061719]" />
                    <div>
                      <span className="font-bold text-white block text-sm">{language === 'ar' ? p.name_ar : p.name_en}</span>
                      <span className="text-[10px] text-[#8EA8AA]">{p.category_slug}</span>
                    </div>
                  </td>
                  <td className="p-4">
                    <span className="font-bold text-[#6CC6C9] block">{p.origin_country_ar}</span>
                    <span className="text-[10px] text-[#8EA8AA]">{p.process_ar}</span>
                  </td>
                  ...
                  <td className="p-4 text-end space-x-2 space-x-reverse">
                    <button
                      ...
                      className="p-1.5 rounded-lg bg-[#0B2528] text-white hover:bg-[#0E5257] transition cursor-pointer inline-block"
                      title={t('تعديل', 'Edit')}
                    >
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-xl bg-[#061719] text-white border border-[#0B2528] rounded-3xl p-6 shadow-2xl z-50 max-h-[90vh] overflow-y-auto">
            
            <button
              onClick={() => setShowModal(false)}
              className="absolute top-4 right-4 p-1 text-[#8EA8AA] hover:text-white cursor-pointer"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <form onSubmit={handleSaveProduct} className="space-y-4 text-xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('الاسم بالعربية', 'Arabic Name')}</label>
                  <input
                    type="text"
                    required
                    value={nameAr}
                    ...
                    className="w-full bg-[#0A1E20] border border-[#0B2528] rounded-xl p-2.5 text-white focus:outline-none focus:border-[#6CC6C9]"
                  />
                </div>

                <div>
                  <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('الاسم بالإنجليزية', 'English Name')}</label>
                  <input
                    type="text"
                    value={nameEn}
                ...
                <div>
                  <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('السعر (﷼)', 'Price (SAR)')}</label>
                  <input
                    type="number"
                    required
                    value={price}
                ...
                <div>
                  <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('سعر التخفيض (اختياري)', 'Sale Price (Optional)')}</label>
                  <input
                    type="number"
                    value={salePrice || ''}
                ...
                <div>
                  <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('كمية المخزون', 'Stock Quantity')}</label>
                  <input
                    type="number"
                    required
                    value={stock}
                ...
                <div>
                  <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('بلد المنشأ', 'Origin Country')}</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('نوع المعالجة', 'Process')}</label>
                  <input
                    type="text"
                    required
                    value={processAr}
                ...
                <div>
                  <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('الإيحاءات (مفصولة بفاصلة)', 'Tasting Notes')}</label>
                  <input
                    type="text"
                    required
              ...
              <button
                type="submit"
                className="w-full bg-[#0E5257] hover:bg-[#2B7D82] text-white py-3 rounded-xl text-xs font-bold transition shadow-lg cursor-pointer"
              >
