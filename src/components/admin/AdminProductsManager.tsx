...
export const AdminProductsManager: React.FC = () => {
 ...
 {filtered.map(p => (
 <tr key={p.id} className="hover:bg-[#FFFFFF]/50 transition">
 <td className="p-4 flex items-center gap-3">
 <img src={p.images[0]} alt="" className="w-10 h-10 rounded-xl object-cover bg-[#FFFFFF]" />
 <div>
 <span className="font-bold text-[#1A2E30] block text-sm">{language === 'ar' ? p.name_ar : p.name_en}</span>
 <span className="text-[10px] text-[#6B8C8E]">{p.category_slug}</span>
 </div>
 </td>
 <td className="p-4">
 <span className="font-bold text-[#6CC6C9] block">{p.origin_country_ar}</span>
 <span className="text-[10px] text-[#6B8C8E]">{p.process_ar}</span>
 </td>
 <td className="p-4 font-bold text-[#1A2E30]">
 ...
 className="p-1.5 rounded-lg bg-[#E8F2F2] text-[#1A2E30] hover:bg-[#0E5257] transition cursor-pointer inline-block"
 title={t('تعديل', 'Edit')}
 >
 ...
 className="p-1.5 rounded-lg bg-red-500/20 text-red-400 hover:bg-red-600 hover:text-[#6CC6C9] transition cursor-pointer inline-block"
 title={t('حذف', 'Delete')}
 >
 ...
 <div className="relative w-full max-w-xl bg-[#FFFFFF] text-[#1A2E30] border border-[#E8F2F2] rounded-3xl p-6 shadow-2xl z-50 max-h-[90vh] overflow-y-auto">
 
 <button
 onClick={() => setShowModal(false)}
 className="absolute top-4 right-4 p-1 text-[#6B8C8E] hover:text-[#6CC6C9] cursor-pointer"
 >
 <X className="w-5 h-5" />
 </button>

 <form onSubmit={handleSaveProduct} className="space-y-4 text-xs">
 <h3 className="font-extrabold text-base text-[#1A2E30]">
 {editingProduct ? t('تعديل محصول/منتج', 'Edit Product') : t('إضافة محصول جديد', 'Add New Product')}
 </h3>

 <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
 <div>
 <label className="block text-[#4A6869] mb-1 font-semibold">{t('الاسم بالعربية', 'Arabic Name')}</label>
 <input
 type="text"
 required
 value={nameAr}
 ...
 <div>
 <label className="block text-[#4A6869] mb-1 font-semibold">{t('الاسم بالإنجليزية', 'English Name')}</label>
 <input
 type="text"
 value={nameEn}
 ...
 <div>
 <label className="block text-[#4A6869] mb-1 font-semibold">{t('السعر (﷼)', 'Price (SAR)')}</label>
 <input
 type="number"
 required
 value={price}
 ...
 <div>
 <label className="block text-[#4A6869] mb-1 font-semibold">{t('سعر التخفيض (اختياري)', 'Sale Price (Optional)')}</label>
 <input
 type="number"
 value={salePrice || ''}
 ...
 <div>
 <label className="block text-[#4A6869] mb-1 font-semibold">{t('كمية المخزون', 'Stock Quantity')}</label>
 <input
 type="number"
 required
 value={stock}
 ...
 <div>
 <label className="block text-[#4A6869] mb-1 font-semibold">{t('بلد المنشأ', 'Origin Country')}</label>
 <input
 type="text"
 required
 ...
 <div>
 <label className="block text-[#4A6869] mb-1 font-semibold">{t('نوع المعالجة', 'Process')}</label>
 <input
 type="text"
 required
 value={processAr}
 ...
 <div>
 <label className="block text-[#4A6869] mb-1 font-semibold">{t('الإيحاءات (مفصولة بفاصلة)', 'Tasting Notes')}</label>
 <input
 type="text"
 required
