    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-[#8EA8AA] mt-0.5">{t('إظهار/إخفاء وإعادة ترتيب أقسام الواجهة الرئيسية', 'Toggle visibility and reorder homepage sections')}</p>
        </div>

        <button
          onClick={handleSave}
          disabled={saving}
          className="bg-[#0E5257] hover:bg-[#2B7D82] text-white px-5 py-2.5 rounded-xl text-xs font-bold transition flex items-center gap-1.5 cursor-pointer shadow-lg"
        >
      ...
      <div className="space-y-2">
        {sections.map((sec, index) => (
          <div key={sec.id} className={`p-4 rounded-2xl border flex items-center justify-between transition ${
            sec.is_enabled ? 'bg-[#0A1E20] border-[#0B2528]' : 'bg-[#0A1E20]/50 border-[#0B2528]/50 opacity-60'
          }`}>
            <div className="flex items-center gap-3">
              <GripVertical className="w-4 h-4 text-[#8EA8AA]" />
              <div className="p-2.5 rounded-xl bg-[#061719] text-[#6CC6C9]">
              ...
              <div>
                ...
                <span className="text-[10px] text-[#8EA8AA] block font-mono">{sec.type} · {t('الترتيب', 'Order')}: {sec.sort_order}</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={index === 0}
                className="p-1.5 rounded-lg bg-[#061719] text-[#8EA8AA] hover:text-[#6CC6C9] disabled:opacity-30 transition cursor-pointer"
              >
