  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-xs text-[#8EA8AA] mt-0.5">{t('رسائل العملاء من نموذج التواصل', 'Customer messages from the contact form')}</p>
      </div>
      ...
      <div className="space-y-3">
        {messages.length === 0 ? (
          ...
          <div key={msg.id} className="p-5 rounded-2xl bg-[#0A1E20] border border-[#0B2528]">
            <div className="flex justify-between items-start">
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 mt-1 text-xs text-[#8EA8AA]">
                ...
                <p className="text-[#B9D0D1] text-xs mt-1">{msg.message}</p>
                {msg.reply_ar && (
                  <div className="mt-2 p-2.5 rounded-lg bg-[#061719] border border-[#0B2528] text-xs space-y-1">
                    <span className="text-[#6CC6C9] font-bold block">{t('ردك:', 'Your reply:')}</span>
                    <p className="text-[#B9D0D1]">{language === 'ar' ? msg.reply_ar : msg.reply_en}</p>
                    <span className="text-[10px] text-[#8EA8AA] block">
                ...
                <div className="mt-2 flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-[#061719] border border-[#0B2528] rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-[#6CC6C9]"
                    onKeyDown={e => {
                      if (e.key === 'Enter') {
                        e.preventDefault();
                        handleReply(msg.id);
                ...
                <span className="text-[10px] text-[#8EA8AA] mt-2 block">
