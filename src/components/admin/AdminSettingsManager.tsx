      ...
      fields: (
        <>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('اسم المتجر بالعربي', 'Arabic Store Name')}</label>
            ...
            <div>
              <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('اسم المتجر بالإنجليزي', 'English Store Name')}</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('رقم ضريبة القيمة المضافة', 'VAT Number')}</label>
              <input type="text" value={settings.vat_number} onChange={e => updateField('vat_number', e.target.value)}
                className="w-full bg-[#061719] border border-[#0B2528] rounded-xl p-2.5 text-white" />
            </div>
            <div>
              <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('نسبة ضريبة القيمة المضافة %', 'VAT Rate %')}</label>
    ...
    },
    {
      ...
      fields: (
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('العملة الافتراضية', 'Default Currency')}</label>
    ...
    },
    {
      ...
      fields: (
        <>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('رقم الهاتف', 'Support Phone')}</label>
              <input type="tel" value={settings.support_phone} onChange={e => updateField('support_phone', e.target.value)}
                className="w-full bg-[#061719] border border-[#0B2528] rounded-xl p-2.5 text-white" />
            </div>
            <div>
              <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('البريد الإلكتروني', 'Support Email')}</label>
              <input type="email" value={settings.support_email} onChange={e => updateField('support_email', e.target.value)}
                className="w-full bg-[#061719] border border-[#0B2528] rounded-xl p-2.5 text-white" />
            </div>
          </div>
          <div>
            <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('رقم واتساب', 'WhatsApp Number')}</label>
            <input type="tel" value={settings.whatsapp_number} onChange={e => updateField('whatsapp_number', e.target.value)}
              className="w-full bg-[#061719] border border-[#0B2528] rounded-xl p-2.5 text-white" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('العنوان بالعربي', 'Arabic Address')}</label>
              <textarea rows={2} value={settings.address_ar} onChange={e => updateField('address_ar', e.target.value)}
                className="w-full bg-[#061719] border border-[#0B2528] rounded-xl p-2.5 text-white resize-none" />
            </div>
            <div>
              <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('العنوان بالإنجليزي', 'English Address')}</label>
      ...
      fields: (
        <div className="space-y-3">
          {[
            ...
            <div key={s.field}>
              <label className="block text-[#B9D0D1] mb-1 font-semibold">{s.label}</label>
      ...
      fields: (
        <>
          <div className="flex items-center justify-between">
            <label className="text-[#B9D0D1] font-semibold">{t('تفعيل برنامج الولاء', 'Enable Loyalty Program')}</label>
          ...
          {settings.enable_loyalty && (
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('نقاط لكل ﷼', 'Points per SAR')}</label>
                <input type="number" value={settings.points_per_sar}
                  onChange={e => updateField('points_per_sar', Number(e.target.value))}
                  className="w-full bg-[#061719] border border-[#0B2528] rounded-xl p-2.5 text-white" />
              </div>
              <div>
                <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('﷼ لكل نقطة', 'SAR per Point')}</label>
    ...
    return (
      <div className="space-y-6 animate-fade-in">
        <div>
          ...
          <p className="text-xs text-[#8EA8AA] mt-0.5">{t('إعدادات المتجر العامة', 'General store configuration')}</p>
        </div>
        <div className="space-y-4">
          {[1, 2, 3].map(i => (
            <div key={i} className="p-6 rounded-3xl bg-[#0A1E20] border border-[#0B2528] animate-pulse space-y-4">
              <div className="h-5 bg-[#0B2528] rounded w-1/4" />
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <form onSubmit={handleSave} className="space-y-4">
        {sections.map(section => (
          <div key={section.titleEn} className="p-6 rounded-3xl bg-[#0A1E20] border border-[#0B2528] space-y-4">
