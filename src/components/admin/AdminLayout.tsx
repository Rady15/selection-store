...
export const AdminLayout: React.FC<AdminLayoutProps> = ({
  ...
  return (
    <div className="bg-[#061719] text-[#F3FBFB] min-h-screen flex flex-col md:flex-row font-sans overflow-x-hidden">

      {/* Mobile Top Bar */}
      <header className="md:hidden sticky top-0 z-30 flex items-center justify-between gap-3 px-4 py-3 bg-[#061719] border-b border-[#0B2528]">
        <img src="/whitelogo.png" alt="Selection Store" className="h-7 w-auto" />
        <div className="flex items-center gap-2">
          <button
            onClick={toggleLanguage}
            className="bg-[#0A1E20] hover:bg-[#0B2528] text-[#B9D0D1] px-3 py-1.5 rounded-lg text-[11px] font-bold border border-[#0B2528] transition flex items-center gap-1.5 cursor-pointer"
          >
          ...
          <button
            ...
            className="bg-[#0A1E20] hover:bg-[#0B2528] text-[#B9D0D1] p-2 rounded-lg border border-[#0B2528] transition cursor-pointer"
      ...
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 ${language === 'ar' ? 'right-0' : 'left-0'} z-50 w-72 max-w-[85vw] bg-[#061719] border-[#0B2528] flex flex-col justify-between flex-shrink-0 transition-transform duration-300 ease-in-out ${language === 'ar' ? 'border-l' : 'border-r'} border-b md:border-b-0 ${mobileNavOpen ? 'translate-x-0' : language === 'ar' ? 'translate-x-full' : '-translate-x-full'} md:sticky md:top-0 md:h-screen md:translate-x-0 md:w-60`}>
        ...
        <div className="flex flex-col min-h-0 flex-1">
          <div className="p-4 border-b border-[#0B2528] flex items-center justify-between">
            ...
            <button
              onClick={closeMobileNav}
              className="md:hidden p-1.5 rounded-lg text-[#8EA8AA] hover:text-white hover:bg-[#0A1E20] transition cursor-pointer"
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-[11px] font-bold transition cursor-pointer ${
                    active
                      ...
                      : 'text-[#8EA8AA] hover:text-white hover:bg-[#0A1E20]'
                  }`}
                >
        ...
        <div className="p-3 border-t border-[#0B2528] space-y-1.5">
          <button
            onClick={toggleLanguage}
            className="w-full bg-[#0A1E20] hover:bg-[#0B2528] text-[#B9D0D1] py-2 rounded-lg text-[11px] font-bold transition flex items-center justify-center gap-2 cursor-pointer border border-[#0B2528]"
          >
            <Globe className="w-3 h-3 text-[#6CC6C9]" />
            <span>{language === 'ar' ? 'EN' : 'AR'}</span>
          </button>
