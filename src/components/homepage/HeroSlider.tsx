...
export const HeroSlider: React.FC<HeroSliderProps> = ({ slides, onNavigate }) => {
  ...
  return (
    <div className="relative w-full min-h-[500px] sm:min-h-[600px] bg-[#061719] overflow-hidden flex items-center justify-center border-b border-[#0B2528]">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-[#061719] via-[#061719]/70 to-[#061719]/30" />
        </motion.div>
      </AnimatePresence>
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 text-center space-y-4 sm:space-y-5 py-12 sm:py-16">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {/* Subtitle */}
            <p className="text-xs sm:text-lg text-[#B9D0D1] max-w-2xl mx-auto leading-relaxed px-2">
      ...
      {slides.length > 1 && (
        <>
          <button
            ...
            className="absolute top-1/2 left-2 sm:left-4 -translate-y-1/2 p-2 sm:p-2.5 rounded-full bg-[#0A1E20]/80 hover:bg-[#0E5257] text-white border border-[#0B2528] transition cursor-pointer z-20 backdrop-blur"
          ...
          <button
            ...
            className="absolute top-1/2 right-2 sm:right-4 -translate-y-1/2 p-2 sm:p-2.5 rounded-full bg-[#0A1E20]/80 hover:bg-[#0E5257] text-white border border-[#0B2528] transition cursor-pointer z-20 backdrop-blur"
          ...
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2 z-20">
            {slides.map((_, idx) => (
              <button
                ...
                className={`h-2 rounded-full transition-all cursor-pointer ${currentIdx === idx ? 'w-8 bg-[#6CC6C9]' : 'w-2 bg-[#0B2528]'
                  }`}
              />
            ))}
          </div>
