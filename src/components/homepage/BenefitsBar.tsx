...
export const BenefitsBar: React.FC<BenefitsBarProps> = ({ items }) => {
  ...
  return (
    <motion.div
      ...
      className="bg-[#0A1E20] border-b border-[#0B2528] py-8"
    >
          ...
          {benefits.map((b, idx) => {
            ...
            return (
              <motion.div
                ...
                className="flex items-center gap-4 p-4 rounded-2xl bg-[#061719]/80 border border-[#0B2528] hover:border-[#6CC6C9]/40 transition group shadow-lg"
              >
                <div className="p-3 rounded-xl bg-[#0E5257]/20 text-[#6CC6C9] group-hover:scale-110 transition flex-shrink-0">
                  <Icon className="w-6 h-6" />
                </div>
                <div>
                  <h4 className="font-bold text-sm text-[#F3FBFB] group-hover:text-[#6CC6C9] transition">
