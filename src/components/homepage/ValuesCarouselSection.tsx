...
export const ValuesCarouselSection: React.FC<ValuesCarouselSectionProps> = ({ items }) => {
  ...
  return (
    <motion.section
      ...
      className="py-14 bg-[#071B1D] border-b border-[#0B2528] relative overflow-hidden"
    >
      {/* Ambient glowing orb */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-[#0E5257]/10 blur-[120px] rounded-full pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6 relative z-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          ...
          >
            <div className="flex items-center gap-1.5 bg-[#061719] p-1 rounded-2xl border border-[#0B2528]">
              <button
                onClick={() => handleScroll('left')}
                className="w-9 h-9 rounded-xl bg-[#0A1E20] border border-[#0B2528] hover:border-[#6CC6C9] text-[#D8E8E9] hover:text-[#6CC6C9] transition flex items-center justify-center cursor-pointer shadow-md active:scale-95"
              >
          ...
          {values.map((v, idx) => {
            ...
            return (
              <motion.div
                ...
                className="w-[280px] sm:w-[320px] flex-shrink-0 scroll-snap-start p-6 rounded-3xl bg-[#061719] border border-[#0B2528] hover:border-[#6CC6C9] transition duration-300 shadow-xl space-y-4 flex flex-col justify-between group transform hover:-translate-y-1"
              >
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="w-12 h-12 rounded-2xl bg-[#0E5257]/20 text-[#6CC6C9] border border-[#6CC6C9]/30 flex items-center justify-center group-hover:scale-110 transition">
                      <Icon className="w-6 h-6" />
                    </div>
                    <span className="text-[10px] font-bold bg-[#0A1E20] text-[#6CC6C9] px-2.5 py-1 rounded-full border border-[#0B2528]">
                      {t(v.badge_ar, v.badge_en)}
                    </span>
                  </div>

                  <h3 className="font-extrabold text-base text-white group-hover:text-[#6CC6C9] transition font-serif">
                ...
                <div className="pt-3 border-t border-[#0B2528]/60 flex items-center justify-between text-[11px] text-[#B9D0D1]">
