...
export const TestimonialsSection: React.FC<TestimonialsSectionProps> = ({ items }) => {
  ...
  return (
    <motion.section
      ...
      className="py-14 bg-[#061719] border-b border-[#0B2528]/40"
    >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {testimonials.map((item, idx) => (
            <motion.div
              ...
              className="p-6 rounded-3xl bg-[#0A1E20] border border-[#0B2528] space-y-4 relative flex flex-col justify-between group hover:border-[#6CC6C9]/50 transition duration-300 shadow-xl"
            >
              ...
              <div className="space-y-3">
                ...
                <p className="text-xs text-[#B9D0D1] leading-relaxed italic">
                  "{t(item.comment_ar, item.comment_en)}"
                </p>
              </div>

              <div className="pt-3 border-t border-[#0B2528]/60">
                <h5 className="font-bold text-sm text-white">{t(item.name_ar, item.name_en)}</h5>
                <p className="text-[11px] text-[#8EA8AA]">{t(item.role_ar, item.role_en)}</p>
              </div>
            </motion.div>
          ))}
        </div>
