...
export const ProductCard: React.FC<ProductCardProps> = ({ product, onNavigate, onQuickView }) => {
  ...
  return (
    <div
      ...
      className="group relative bg-[#0A1E20] border border-[#0B2528] hover:border-[#6CC6C9]/60 rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl hover:-translate-y-1 transition duration-500 flex flex-col cursor-pointer brand-glow"
    >
      {/* Top Image Box */}
      <div className="relative aspect-square bg-[#061719] overflow-hidden">
        {/* Primary Image */}
        <img
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[#061719]/80 via-transparent to-transparent opacity-60" />

        {/* Top Right Badges */}
        <div className="absolute top-3 right-3 flex flex-col gap-1.5 z-10 items-end">
          {product.is_new && (
            <span className="bg-[#0E5257] text-white text-[10px] font-extrabold px-2 py-0.5 rounded-md shadow uppercase">
              {t('جديد', 'NEW')}
            </span>
          )}
          {product.is_bestseller && (
            <span className="bg-[#6CC6C9] text-black text-[10px] font-extrabold px-2 py-0.5 rounded-md shadow uppercase">
        ...
        <button
          ...
          className={`absolute top-3 left-3 p-2 rounded-full backdrop-blur-md transition z-10 cursor-pointer ${isLiked
              ...
              : 'bg-[#061719]/60 text-[#B9D0D1] hover:text-white hover:bg-[#0E5257]'
            }`}
        ...
        {onQuickView && (
          <button
            ...
            className="absolute bottom-3 left-1/2 -translate-x-1/2 bg-[#061719]/90 hover:bg-[#0E5257] text-white px-3.5 py-1.5 rounded-xl text-xs font-bold transition opacity-0 group-hover:opacity-100 shadow-xl flex items-center gap-1.5 z-10 cursor-pointer"
          >
      ...
      <div className="p-4 flex-1 flex flex-col justify-between space-y-3">
        ...
        <div>
          ...
          <div className="flex items-center gap-1.5 mb-1.5 flex-wrap">
            <span className="text-[10px] font-bold bg-[#0E5257]/20 text-[#6CC6C9] px-2 py-0.5 rounded">
          ...
          {/* Title */}
          <h3 className="font-bold text-sm sm:text-base text-[#F3FBFB] group-hover:text-[#6CC6C9] transition line-clamp-1">
          ...
          <div className="flex items-center gap-1 mt-2 flex-wrap">
            {(language === 'ar' ? product.tasting_notes_ar : product.tasting_notes_en).slice(0, 3).map((note, idx) => (
              <span
                key={idx}
                className="text-[10px] bg-[#061719] text-[#B9D0D1] px-2 py-0.5 rounded border border-[#0B2528]"
              >
                {note}
              </span>
            ))}
        ...
        <div className="pt-2 border-t border-[#0B2528] space-y-2">

          {/* Rating & Stock */}
          <div className="flex items-center justify-between text-xs text-[#8EA8AA]">
            <div className="flex items-center gap-1 text-[#6CC6C9] font-bold">
              <Star className="w-3.5 h-3.5 fill-current" />
              <span>{product.rating}</span>
              <span className="text-[10px] text-[#8EA8AA]">({product.review_count})</span>
            </div>
          ...
          <div className="flex items-center justify-between gap-2">
            <div>
              <div className="font-extrabold text-base sm:text-lg text-[#6CC6C9]">
                {formatPrice(currentPrice)}
              </div>
              {hasDiscount && (
                <div className="text-[11px] text-[#8EA8AA] line-through">
