...
export const SearchOverlay: React.FC<SearchOverlayProps> = ({ isOpen, onClose, onNavigate }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-[#061719]/95 backdrop-blur-xl text-[#F3FBFB] overflow-y-auto animate-fade-in">

      {/* Top Search Input Bar */}
      <div className="p-4 sm:p-6 border-b border-[#0B2528] bg-[#061719] sticky top-0 z-10 shadow-2xl">
        <div className="max-w-4xl mx-auto flex items-center gap-3">
          ...
          <input
            ...
            className="w-full bg-transparent text-lg sm:text-2xl font-bold placeholder-[#8EA8AA]/50 text-[#F3FBFB] focus:outline-none"
          />
          ...
          {loading ? (
            ...
            <button
              onClick={() => setQuery('')}
              className="p-1 rounded-full text-[#8EA8AA] hover:text-white hover:bg-[#0A1E20] cursor-pointer"
            >
              <X className="w-5 h-5" />
            </button>
          ) : null}
          ...
          <button
            onClick={onClose}
            className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-[#0A1E20] hover:bg-[#0B2528] text-[#B9D0D1] hover:text-white transition cursor-pointer border border-[#0B2528]"
          >
      ...
      <div className="max-w-4xl mx-auto w-full p-4 sm:p-6 flex-1">
        ...
        {!query.trim() && (
          <div className="space-y-8 py-4">
              ...
              <div className="flex flex-wrap gap-2">
                {trendingKeywords.map(kw => (
                  <button
                    ...
                    className="px-3.5 py-1.5 rounded-xl bg-[#0A1E20] hover:bg-[#0E5257]/30 border border-[#0B2528] hover:border-[#6CC6C9]/50 text-xs font-medium text-[#D8E8E9] hover:text-[#6CC6C9] transition cursor-pointer"
                  >
                    {kw}
                  </button>
                ))}
            ...
            {recentSearches.length > 0 && (
              <div>
                <div className="flex items-center justify-between text-xs font-bold text-[#8EA8AA] uppercase tracking-wider mb-3">
                  <div className="flex items-center gap-2">
                    <Tag className="w-4 h-4 text-[#8EA8AA]" />
                ...
                <div className="flex flex-wrap gap-2">
                  {recentSearches.map(kw => (
                    <button
                      key={kw}
                      onClick={() => handleSelectKeyword(kw)}
                      className="px-3 py-1 rounded-lg bg-[#061719] hover:bg-[#0A1E20] border border-[#0B2528] text-xs text-[#8EA8AA] hover:text-white transition cursor-pointer"
                    >
                      {kw}
                    </button>
                  ))}
        ...
        {query.trim() && (
          <div className="space-y-4">
            <div className="flex items-center justify-between border-b border-[#0B2528] pb-3 text-xs text-[#8EA8AA]">
              <span>
            ...
            {results.length === 0 && !loading ? (
              <div className="text-center py-16 space-y-3">
                <Coffee className="w-12 h-12 text-[#0B2528] mx-auto" />
                <h3 className="font-bold text-lg text-[#F3FBFB]">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                {results.map(prod => (
                  <div
                    ...
                    className="group p-3 rounded-2xl bg-[#0A1E20]/80 hover:bg-[#0A1E20] border border-[#0B2528] hover:border-[#6CC6C9]/50 transition cursor-pointer flex gap-3.5 items-center"
                  >
                    <div className="w-20 h-20 rounded-xl overflow-hidden bg-[#061719] relative flex-shrink-0">
                      <img
                        src={prod.images[0]}
                        alt={prod.name_ar}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <h4 className="font-bold text-sm text-[#F3FBFB] group-hover:text-[#6CC6C9] transition truncate">
