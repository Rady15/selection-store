  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* sparkle badge */}
        <span className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-[#F3FBFB] text-[#0E5257] text-[10px] font-extrabold flex items-center justify-center shadow-lg border border-[#6CC6C9]">
          ✨
        </span>
      </button>
      ...
      >
        <div className={`rounded-2xl bg-[#0A1E20]/95 backdrop-blur-md border border-[#6CC6C9]/50 px-4 py-2.5 shadow-2xl whitespace-nowrap animate-fab-label ${hovered ? 'border-[#6CC6C9]' : ''}`}>
          <p className="text-xs font-extrabold text-[#6CC6C9] leading-tight">
            {t('مستشار القهوة الذكي', 'Smart Coffee Advisor')}
          </p>
          <p className="text-[10px] text-[#8EA8AA] leading-tight mt-0.5">
            {t('اكتشف قهوتك المثالية في 30 ثانية', 'Find your perfect cup in 30 seconds')}
          </p>
        </div>
        <span className={`absolute top-1/2 -translate-y-1/2 ${language === 'ar' ? 'right-0 translate-x-1/2 border-r border-t' : 'left-0 -translate-x-1/2 border-r border-t'} w-2 h-2 rotate-45 bg-[#0A1E20] border-[#6CC6C9]/50`} />
      </div>
    </div>
  );
};
