...
export const ProductQuickViewModal: React.FC<ProductQuickViewModalProps> = ({ product, onClose, onNavigate }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal Container */}
      <div className="relative w-full max-w-4xl bg-[#061719] text-[#F3FBFB] border border-[#0B2528] rounded-3xl shadow-2xl z-50 max-h-[90vh] overflow-y-auto grid grid-cols-1 md:grid-cols-2 p-6 gap-6">

        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-10 p-2 rounded-full bg-[#0A1E20] text-[#8EA8AA] hover:text-white hover:bg-[#0B2528] transition cursor-pointer"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="space-y-3">
          <div className="aspect-square rounded-2xl bg-[#0A1E20] overflow-hidden border border-[#0B2528] relative">
            <img
            ...
            <div className="flex gap-2">
              {product.images.map((img, idx) => (
                <button
                  ...
                  className={`w-16 h-16 rounded-xl overflow-hidden border-2 transition cursor-pointer ${activeImageIdx === idx ? 'border-[#6CC6C9]' : 'border-[#0B2528] opacity-60'
                    }`}
                >
          ...
          {product.flavor_profile && (
            <div className="p-3 bg-[#0A1E20] border border-[#0B2528] rounded-xl">
        ...
        <div className="flex flex-col justify-between space-y-4">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-[#B9D0D1] font-medium">
                {language === 'ar' ? product.process_ar : product.process_en}
              </span>
            </div>

            {/* Title */}
            <h3 className="font-extrabold text-xl text-[#F3FBFB]">
            ...
            <div className="flex items-baseline gap-2 pt-1">
              ...
              {product.sale_price && (
                <span className="text-xs text-[#8EA8AA] line-through">
            ...
            <div className="space-y-1">
              <span className="text-xs font-bold text-[#8EA8AA] uppercase">{t('الإيحاءات', 'Tasting Notes')}:</span>
              <div className="flex flex-wrap gap-1.5 pt-1">
                {(language === 'ar' ? product.tasting_notes_ar || [] : product.tasting_notes_en || []).map((note, i) => (
                  <span key={i} className="text-xs bg-[#0A1E20] text-[#6CC6C9] px-2.5 py-1 rounded-lg border border-[#0B2528] font-medium">
                    {note}
                  </span>
                ))}
              </div>
            ...
            {(product.weight_options || []).length > 0 && (
              <div className="space-y-1.5 pt-2">
                <span className="text-xs font-bold text-[#8EA8AA]">{t('اختر الوزن', 'Select Weight')}:</span>
            ...
            {(product.grind_options || []).length > 0 && (
              <div className="space-y-1.5 pt-2">
                <span className="text-xs font-bold text-[#8EA8AA]">{t('درجة الطحن', 'Grind Type')}:</span>
                <select
                  value={selectedGrind}
                  onChange={e => setSelectedGrind(e.target.value as GrindType)}
                  className="w-full bg-[#0A1E20] text-[#F3FBFB] border border-[#0B2528] rounded-xl p-2.5 text-xs focus:outline-none focus:border-[#6CC6C9] cursor-pointer"
                >
          ...
          <div className="pt-4 border-t border-[#0B2528] space-y-2">

            <div className="flex gap-2">
              {/* Quantity */}
              <div className="flex items-center border border-[#0B2528] rounded-xl bg-[#0A1E20] px-2">
                <button
