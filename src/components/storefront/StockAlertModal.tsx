  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-[#061719] text-[#F3FBFB] border border-[#0B2528] rounded-3xl p-6 shadow-2xl z-50">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-1.5 rounded-full bg-[#0A1E20] text-[#8EA8AA] hover:text-white cursor-pointer"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        {success ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-3 text-xs">
              ...
              <div>
                <label className="block text-[#B9D0D1] mb-1 font-semibold">{t('رقم الجوال (لإرسال SMS)', 'Phone Number (For SMS)')}</label>
                <input
                  type="tel"
                  required
                  value={phone}
