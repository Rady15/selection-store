...
export const QuestionsSection: React.FC<QuestionsSectionProps> = ({ productId }) => {
  ...
  return (
    <div className="space-y-6 pt-4">
      {/* Submit Question Box */}
      <div className="p-5 bg-[#0A1E20] border border-[#0B2528] rounded-3xl space-y-3">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            <input
              ...
              className="bg-[#061719] border border-[#0B2528] rounded-xl p-2.5 text-xs text-white focus:outline-none focus:border-[#6CC6C9]"
            />
            <input
              type="text"
              required
              ...
              className="sm:col-span-2 bg-[#061719] border border-[#0B2528] rounded-xl p-2.5 text-xs text-white focus:outline-none focus:border-[#6CC6C9]"
            />
          </div>
          ...
          <div className="flex justify-end">
            <button
              type="submit"
              disabled={submitting}
              className="bg-[#0E5257] hover:bg-[#2B7D82] text-white px-5 py-2 rounded-xl text-xs font-bold transition flex items-center gap-1.5 cursor-pointer"
            >
      ...
      <div className="space-y-3">
        {questions.length === 0 ? (
          ...
          questions.map(q => (
            <div key={q.id} className="p-4 bg-[#0A1E20] border border-[#0B2528] rounded-2xl space-y-2">
              <div className="flex items-center gap-2 text-xs text-[#6CC6C9] font-bold">
                <MessageCircle className="w-4 h-4" />
                <span>{q.customer_name}: {q.question}</span>
              </div>

              {q.answer_ar ? (
                <div className="p-3 bg-[#061719] border-s-2 border-emerald-500 rounded-xl text-xs space-y-1">
                  <span className="font-bold text-emerald-400">{t('إجابة محامص سليكشن الرسمية:', 'Official Selection Answer:')}</span>
                  <p className="text-[#B9D0D1]">{language === 'ar' ? q.answer_ar : q.answer_en}</p>
                </div>
              ) : (
                <span className="text-[10px] text-[#8EA8AA] italic">
